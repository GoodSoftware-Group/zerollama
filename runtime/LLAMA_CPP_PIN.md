...
| **Ollama patches** | `llama/patches/` via `Makefile.sync` + `./scripts/vendor/sync_vendor_llama.sh` (through **0101** Kokoro `/v1/audio/speech`; **0100** Kokoro subtree; **0099** FORCE_CUBLAS getenv; **0098** DCA LSE tile+budget; **0097** native DCA graph; **0096** FA LSE export; **0095** dca hparams; **0094** GGUF dca keys; **0090** media-aware `/kv/seq-copy`; **0089** L3-R6b cell+tensor+pages COW; **0088** TBQ vec_dot; **0087** Bee loop-guard); container build: `./scripts/vendor/build_llama_server_container.sh`) |
