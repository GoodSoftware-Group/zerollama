...
| `vram_autotune` | object | `enabled`, factors, `persist` (incl. `catalog[]`: `model`, `basename`, `estimate_factor`, `last`), `export`, `apply_exported_env` (startup apply status). Autotune persist wins per GGUF over applied env. `pending_first_calibration`. |
...
| `vram_estimate` | object | Loaded model or `LLAMA_MODEL` when set; uses `loaded_vram_num_ctx` when llama-server is up. Includes `kv_block_layout`, `kv_bytes_*`, `head_dim_source`; IQ/TQ KV types floor at F16 bytes/element. |
...
| `autoconfig` | object | `config_path`, `pick` (`single_gpu` / `dual_4090` / `custom`), `visible_gpu_count`, optional `gpu_total_vram`. |
