...
| `vram_autotune` | object | `enabled`, factors, `persist`, `export`, `apply_exported_env` (startup apply status). Autotune persist wins per GGUF over applied env. `pending_first_calibration`. |
