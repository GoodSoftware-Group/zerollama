...
func buildModelEntries(modelList []LaunchModel) map[string]any {
	...
	for _, launchModel := range modelList {
		...
		if launchModel.HasCapability(model.CapabilityThinking) {
			...
			if openCodeModelSupportsThinkingLevels(launchModel) {
		...
		if launchModel.MaxOutputTokens > 0 {
			...
			if launchModel.ContextLength > 0 {
				limit["context"] = launchModel.ContextLength
			}
			limit["output"] = launchModel.MaxOutputTokens
			entry["limit"] = limit
		} else if launchModel.ContextLength > 0 {
			entry["limit"] = map[string]any{
				"context": launchModel.ContextLength,
			}
		}
		models[launchModel.Name] = entry
	}
	return models
}

func openCodeModelSupportsThinkingLevels(launchModel LaunchModel) bool {
	for _, family := range append([]string{launchModel.Details.Family}, launchModel.Details.Families...) {
		if normalizeOpenCodeModelFamily(family) == "gptoss" {
			return true
		}
	}
	return strings.Contains(normalizeOpenCodeModelFamily(launchModel.Name), "gptoss")
}
