...
func TestCreateConfig(t *testing.T) {
	...
	t.Run("cloud model without limit metadata omits context window", func(t *testing.T) {
		cfg := createConfig(fallbackLaunchModel("glm-5:cloud"))
		if _, ok := cfg["contextWindow"]; ok {
			t.Errorf("contextWindow = %v, want unset without cloud limit metadata", cfg["contextWindow"])
		}
	})
