...
//
// WHY not pass bare []string: integrations need vision/thinking flags, context
// length, and cloud limits when writing OpenCode/OpenClaw/Pi/OMP YAML. Calling
// /api/show per model at Edit time was slow and could disagree with /api/tags.
...
// WithCloudLimits fills context/output from the static cloud limit map when tags
// omitted them. WHY: cloud models may not be in local blob inventory but still
// need sane caps in agent config without a Show round-trip.
...
// modelInventory caches one /api/tags load per launcherClient invocation.
// WHY per-run cache: a single `zerollama launch` may configure then run; re-listing
// on every Resolve would duplicate work. Refresh is explicit on local miss (post-pull).
...
// Resolve maps selected names to LaunchModel entries, refreshing tags once when a
// local name is missing. WHY refresh-on-miss: user may have pulled between picker
// display and configure; cloud names never trigger refresh (not in local inventory).
