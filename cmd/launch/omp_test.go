...
func TestOMPConfigureWritesModelsYML(t *testing.T) {
	home := t.TempDir()
	setOMPTestHome(t, home)
	t.Setenv("OLLAMA_HOST", "http://0.0.0.0:11434")

	o := &OMP{}
	if err := o.Configure("glm-5.1:cloud"); err != nil {
	...
	if entries[0]["id"] != "glm-5.1:cloud" {
		t.Fatalf("first model id = %v, want primary", entries[0]["id"])
	}
	if input := stringSliceYAMLValue(entries[0]["input"]); !slices.Equal(input, []string{"text"}) {
		t.Fatalf("input = %v, want [text]", input)
	}
...
func TestOMPConfigurePreservesExistingConfig(t *testing.T) {
	home := t.TempDir()
	setOMPTestHome(t, home)
...
`)
	...
	o := &OMP{}
	if err := o.Configure("new-model"); err != nil {
		t.Fatalf("Configure returned error: %v", err)
	}
...
func TestOMPConfigureAlwaysMarksSetupComplete(t *testing.T) {
	home := t.TempDir()
	setOMPTestHome(t, home)
...
func TestOMPConfigureRespectsPiConfigDir(t *testing.T) {
	home := t.TempDir()
	setOMPTestHome(t, home)
...
func TestOMPConfigureRespectsPiCodingAgentDir(t *testing.T) {
	home := t.TempDir()
	setOMPTestHome(t, home)
