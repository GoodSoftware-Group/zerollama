...
func TestManagedSingleConfigureModelsUsesInventoryOnly(t *testing.T) {
	...
	got, err := client.managedSingleConfigureModels(context.Background(), &OMP{}, "llama3.2")
	if err != nil {
		t.Fatalf("managedSingleConfigureModels returned error: %v", err)
	}
	want := []string{"llama3.2"}
	if diff := compareStrings(got, want); diff != "" {
		t.Fatalf("managedSingleConfigureModels %s", diff)
	}
}
...
func TestLaunchIntegration_ConfiguredEditorLaunchSkipsReconfigure(t *testing.T) {
	...
	editor := &launcherEditorRunner{paths: []string{"/tmp/settings.json"}, models: []string{"llama3.2", "qwen3:8b"}}
	withIntegrationOverride(t, "droid", editor)
...
func TestLaunchIntegration_ConfiguredEditorLaunchRewritesDriftedLiveConfig(t *testing.T) {
	...
	if len(editor.edited) != 1 {
		t.Fatalf("expected one editor rewrite when live config drifts, got %v", editor.edited)
	}
	if diff := compareStrings(editor.edited[0], []string{"llama3.2", "mistral"}); diff != "" {
		t.Fatalf("unexpected editor rewrite (-want +got):\n%s", diff)
	}
	if editor.ranModel != "llama3.2" {
		t.Fatalf("expected launch to use saved primary model, got %q", editor.ranModel)
	}
