...
// OMP implements Runner and ManagedSingleModel for the OMP coding-agent integration.
type OMP struct{}
...
// ConfigureWithModels writes the full installed catalog to models.yml.
// WHY []LaunchModel: vision/context come from inventory; callers pass
// managedSingleConfigureModels output (tags only, not picker recommendations).
...
func ompModelConfig(modelInfo LaunchModel) map[string]any {
	...
	input := []string{"text"}
	if modelInfo.HasCapability(model.CapabilityVision) {
