...
#
# In-repo: bash scripts/serve_a380_example.sh
...
source "${_ROOT}/scripts/a380_llama_vendor.sh"
...
if a380_export_llama_vendor_env; then
  ...
  echo "serve_a380: warn — using stock /usr/lib/ollama (run ./scripts/build_zerollama_a380.sh)" >&2
fi
...
export OLLAMA_EXTERNAL_IMAGE_BIN="${OLLAMA_EXTERNAL_IMAGE_BIN:-${_ROOT}/scripts/sd_external_image.sh}"
