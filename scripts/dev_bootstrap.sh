...
#   MAC_SETUP_SIGNOFF=1 ./scripts/dev_bootstrap.sh   # after zerollama pull …
#   MAC_SETUP_TRAINING=1 ./scripts/dev_bootstrap.sh
#
# Same env as mac_setup.sh; see scripts/mac_setup.sh for full list.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
export MAC_SETUP_SIGNOFF="${MAC_SETUP_SIGNOFF:-0}"
export MAC_SETUP_LLAMA_CLONE="${MAC_SETUP_LLAMA_CLONE:-1}"
exec "${ROOT}/scripts/mac_setup.sh"
