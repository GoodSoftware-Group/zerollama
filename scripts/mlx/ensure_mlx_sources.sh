...
# (GOFLAGS=-mod=mod ./scripts/build/build_zerollama_mac.sh with BUILD_MLX=1) — or
# build_mlx_dylibs_mac.sh alone after pins change.
#
# Usage:
#   ./scripts/mlx/ensure_mlx_sources.sh
#   ./scripts/mlx/ensure_mlx_sources.sh --clone    # clone missing sibling repos (full history)
#   source ./scripts/mlx/ensure_mlx_sources.sh && ensure_mlx_sources
set -euo pipefail
