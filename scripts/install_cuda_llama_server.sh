#!/usr/bin/env bash
# Install CUDA llama-server + ggml libs built by ./scripts/build_llama_server.sh
# into the Ollama layout under /usr/local/lib/ollama (dual-GPU / RTX 4090 hosts).
#
# Usage:
#   LLAMA_BUILD_WEBUI=OFF ./scripts/build_llama_server.sh
#   sudo ./scripts/install_cuda_llama_server.sh
#
# Env:
...
if [[ ! -x "${BUILD_DIR}/llama-server" ]]; then
  ...
  echo "  ./scripts/build_llama_server_container.sh" >&2
  exit 1
fi
