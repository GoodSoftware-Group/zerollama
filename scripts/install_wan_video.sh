...
# Usage: ./scripts/install_wan_video.sh --profile 1.3b|2.2|all
set -euo pipefail
...
patch_wan_attention() {
  ...
  if [[ -f "$attn" ]]; then
    python3 "$REPO_ROOT/scripts/patch_wan_attention_sdpa.py" "$attn"
  fi
}
patch_wan_attention
...
run_wan_torch_probe() {
  ...
  if ! "$VENV_DIR/bin/python3" "$REPO_ROOT/scripts/wan_torch_compat.py"; then
...
echo "Register models: ./scripts/register_wan_models.sh"
