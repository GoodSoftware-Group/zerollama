...
#
# Usage: ./scripts/install_openvino_diffusion.sh [--models-only]
#
...
chmod +x "$REPO_ROOT/scripts/ov_external_image.sh"

if [[ -d /usr/lib/ollama-zerollama ]]; then
  cp "$REPO_ROOT/scripts/ov_external_image.sh" "$REPO_ROOT/scripts/ov_image_generate.py" /usr/lib/ollama-zerollama/ 2>/dev/null || \
    sudo cp "$REPO_ROOT/scripts/ov_external_image.sh" "$REPO_ROOT/scripts/ov_image_generate.py" /usr/lib/ollama-zerollama/
...
echo "Register: ./scripts/register_ov_models.sh"
