...
#
# Usage:
#   M3_LLAMA_MODEL=/path/to/small.gguf ./scripts/l2_full_gate.sh
...
M3_LLAMA_MODEL="${M3_LLAMA_MODEL:-}" "${ROOT}/scripts/l2_runtime_compat_smoke.sh"
...
if [[ "${L2_SKIP_BENCH:-0}" != "1" ]]; then
  ...
  L2_METAL_BENCH_OUT="${OUT_DIR}/bench-8k.json" \
    L2_NUM_CTX=8192 \
    "${ROOT}/scripts/l2_metal_bench.sh"
