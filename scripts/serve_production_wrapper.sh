...
if [[ ! -f "${ZEROLLAMA_REPO}/scripts/serve_gpu_example.sh" ]]; then
  echo "~/bin/serve.sh: missing ${ZEROLLAMA_REPO}/scripts/serve_gpu_example.sh" >&2
  echo "WHY: set ZEROLLAMA_REPO to your zerollama checkout (symlink ~/zerollama is fine)." >&2
  exit 1
fi

exec bash "${ZEROLLAMA_REPO}/scripts/serve_gpu_example.sh"
