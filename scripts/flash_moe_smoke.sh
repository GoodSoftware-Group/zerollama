...
#   FLASH_MOE_BUILD=1 ./scripts/flash_moe_smoke.sh
#
...
if [[ ! -x "${FLASH_MOE_BIN}" ]]; then
  if [[ "${FLASH_MOE_BUILD:-0}" == "1" ]]; then
    ...
    FLASH_MOE_REPO="${FLASH_MOE_REPO}" "${ROOT}/scripts/build_flash_moe_llama_server.sh"
  else
    echo "flash_moe_smoke: missing ${FLASH_MOE_BIN} (set FLASH_MOE_BUILD=1 or run ./scripts/build_flash_moe_llama_server.sh)" >&2
    exit 1
  fi
fi
...
if [[ -z "${FLASH_MOE_SIDECAR}" ]]; then
  if [[ "${FLASH_MOE_EXTRACT:-0}" == "1" ]]; then
    ...
    echo "hint: FLASH_MOE_EXTRACT=1 or ./scripts/flash_moe_extract_sidecar.sh --model ${FLASH_MOE_GGUF} --out-dir ~/Models/flash/$(basename "${FLASH_MOE_GGUF}" .gguf)" >&2
    exit 1
  fi
fi
