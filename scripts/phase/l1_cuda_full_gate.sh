...
#
# Usage:
#   CUDA_LLAMA_MODEL=/root/eliza-1-9b-256k.gguf ./scripts/phase/l1_cuda_full_gate.sh
#   CUDA_LLAMA_MODEL=/path/to/supernova-fp16.gguf ./scripts/phase/l1_cuda_full_gate.sh  # optional re-validate
#
# Env:
...
if [[ -f "${ROOT}/scripts/gpu/5080_env.sh" ]]; then
  # shellcheck source=scripts/gpu/5080_env.sh disable=SC1091
...
if [[ "${L1_SKIP_CALIBRATE:-0}" != "1" ]]; then
  L1_OUT_DIR="${L1_GATE_DIR}/calibrate" \
    "${ROOT}/scripts/phase/l1_cuda_calibrate.sh"
...
if [[ "${L1_SKIP_CONCURRENT:-0}" != "1" ]]; then
  ...
  L1C_ENFORCE=0 \
    "${ROOT}/scripts/phase/l1_cuda_concurrent_bench.sh"
fi
...
"${ROOT}/scripts/phase/l1_gate_report.sh" "${L1_GATE_OUT}"
