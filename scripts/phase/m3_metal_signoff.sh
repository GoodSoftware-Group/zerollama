...
if [[ ! -f "${LLAMA_CPP_LIB}" ]]; then
  echo "Missing ${LLAMA_CPP_LIB}; run ./scripts/build/build_llama_server.sh" >&2
  exit 1
fi
...
if [[ "${RUN_E2E_QWEN35:-0}" == "1" ]]; then
  echo ""
  "${ROOT}/scripts/runtime/qwen35_mac_smoke.sh"
fi

if [[ "${RUN_E2E_L2:-0}" == "1" ]]; then
  echo ""
  M3_LLAMA_MODEL="$LLAMA_MODEL" "${ROOT}/scripts/phase/l2_full_gate.sh"
fi

if [[ "${RUN_E2E_L3:-0}" == "1" ]]; then
  echo ""
  M3_LLAMA_MODEL="$LLAMA_MODEL" "${ROOT}/scripts/phase/l3_cache_smoke.sh"
  "${ROOT}/scripts/phase/l3_gate_report.sh" "${L3_OUT:-/tmp/l3-cache-smoke.json}"
fi

if [[ "${RUN_E2E_PHASE15:-0}" == "1" ]]; then
  echo ""
  M3_LLAMA_MODEL="$LLAMA_MODEL" PHASE15_SKIP_BOOT=1 "${ROOT}/scripts/phase/phase15_metal_signoff.sh"
fi
