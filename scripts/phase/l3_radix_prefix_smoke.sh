...
#
# Usage:
#   ./scripts/phase/l3_radix_prefix_smoke.sh
# Live (needs local GGUF + patched vendor llama-server with POST /kv/seq-copy):
#   M3_LLAMA_MODEL=/path/model.gguf L3_RADIX_LIVE=1 ./scripts/phase/l3_radix_prefix_smoke.sh
# 5080 / CUDA (CUDA_LLAMA_MODEL alias — same as l3_cache_smoke):
#   CUDA_LLAMA_MODEL=/root/eliza-1-9b-256k.gguf L3_RADIX_LIVE=1 ./scripts/phase/l3_radix_prefix_smoke.sh
set -euo pipefail
...
if server is None:
    raise SystemExit(f"llama-server not found (root={root}); run ./scripts/build/build_llama_server.sh")
...
if [[ ! -x "${LLAMA_SERVER_BIN}" ]]; then
  ...
  echo "  LLAMA_CPP_ROOT=${LLAMA_CPP_ROOT} ./scripts/build/build_llama_server.sh" >&2
  exit 1
fi
...
if not seq_copy_ok:
    raise SystemExit(
        ...
        f"LLAMA_CPP_ROOT={os.environ.get('LLAMA_CPP_ROOT')} ./scripts/build/build_llama_server.sh"
    )
