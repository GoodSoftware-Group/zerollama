...
#   P17_L2_OUT=/tmp/phase17-l2-pin-status.json ./scripts/phase/phase17_l2_pin_status.sh
set -euo pipefail
...
if [[ -d "${UNIFIED_DIR}/.git" ]]; then
  ...
  echo "unified sibling HEAD:                  (not cloned — run ./scripts/build/build_llama_server.sh)"
fi

echo ""
echo "L2 verdict (Jun 2026, documented): FAIL fork-profile merge @ 8k CUDA — L1 q8_0 faster; fork profiles stay opt-in."
echo "Gates: ./scripts/phase/l2_cuda_full_gate.sh (CUDA) · ./scripts/phase/l2_full_gate.sh (Metal)"
...
report = {
    ...
    "gates": ["scripts/phase/l2_cuda_full_gate.sh", "scripts/phase/l2_full_gate.sh"],
}
