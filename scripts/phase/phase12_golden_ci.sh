...
#
#   ./scripts/phase/phase12_golden_ci.sh          # all (GPU preflight / local)
#   ./scripts/phase/phase12_golden_ci.sh go       # Go -run Golden only
#   ./scripts/phase/phase12_golden_ci.sh py       # Python tools meta (needs runtime installed)
#
...
#   RUN_E2E_PREFLIGHT=1 ./scripts/gpu/gpu_smoke_all.sh
set -euo pipefail
...
_run_py_golden() {
  ...
  if ! "$py" -c "import runtime" 2>/dev/null; then
    echo "runtime package not importable; run: ./scripts/runtime/runtime_uv_venv.sh" >&2
    exit 1
  fi
  cd "${ROOT}/runtime"
