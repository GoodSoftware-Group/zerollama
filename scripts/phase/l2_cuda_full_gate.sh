...
#
# Usage:
#   CUDA_LLAMA_MODEL=/path/to/small.gguf ./scripts/phase/l2_cuda_full_gate.sh
#   L2_BUILD_FORK=1 L2_RUN_27K=1 L2_RUN_131K_FORK=1 ./scripts/phase/l2_cuda_full_gate.sh
#
# Env:
...
CUDA_LLAMA_MODEL="${CUDA_LLAMA_MODEL}" "${ROOT}/scripts/phase/l2_cuda_runtime_compat_smoke.sh"
...
if [[ "${L2_SKIP_BENCH:-0}" != "1" ]]; then
  ...
  L2_CUDA_BENCH_OUT="${OUT_DIR}/bench-8k.json" \
    L2_NUM_CTX=8192 \
    "${ROOT}/scripts/phase/l2_cuda_bench.sh"
