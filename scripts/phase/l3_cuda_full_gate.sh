...
#
# Usage:
#   CUDA_LLAMA_MODEL=/root/eliza-1-9b-256k.gguf ./scripts/phase/l3_cuda_full_gate.sh
#   CUDA_LLAMA_MODEL=/path/to/supernova-fp16.gguf ./scripts/phase/l3_cuda_full_gate.sh  # optional re-validate
#
# Env:
...
if [[ "${L3_RAN_SMOKE}" == "1" ]]; then
  ...
  L3_OUT="${SMOKE_JSON}" \
    "${ROOT}/scripts/phase/l3_cache_smoke.sh"
fi
...
if [[ "${L3_RAN_PRODUCTION}" == "1" ]]; then
  ...
  L3_COMPARE_NO_CACHE="${L3_COMPARE_NO_CACHE:-1}" \
    "${ROOT}/scripts/phase/l3_production_gate.sh"
fi
...
if [[ "${L3_RUN_SPEC_CACHE:-0}" == "1" ]]; then
  ...
  L3_OUT="${SPEC_JSON}" \
    "${ROOT}/scripts/phase/l3_spec_cache_smoke.sh"
fi
...
"${ROOT}/scripts/phase/l3_gate_report.sh" "${L3_GATE_OUT}"
