#!/usr/bin/env bash
# Rebase in-process vendor onto ggml-org/llama.cpp @ LLAMA_CPP_COMMIT + Ollama patches.
#
# WHY: runtime llama-server and Go ggml should share one source tree (Ollama pin +
...
#
# Usage:
#   ./scripts/vendor/rebase_vendor_unified.sh          # clone/checkout only (patches pre-applied)
#   ./scripts/vendor/rebase_vendor_unified.sh --sync   # rsync vendor → in-tree ggml + llama.cpp
#   ./scripts/vendor/rebase_vendor_unified.sh --apply  # git am all patches (fresh vendor reset)
set -euo pipefail
...
if [[ "${PATCH_COUNT}" -eq 0 ]]; then
  echo "error: no patch commits on vendor — run with --apply or ./scripts/vendor/ensure_llama_vendor_patches.sh" >&2
  exit 1
fi

if [[ "${DO_SYNC}" == "1" || "${DO_APPLY}" == "1" ]]; then
  "${ROOT}/scripts/vendor/sync_vendor_llama.sh"
fi
