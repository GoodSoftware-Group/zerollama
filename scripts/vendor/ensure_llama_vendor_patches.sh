...
#
# Usage:
#   ./scripts/vendor/ensure_llama_vendor_patches.sh
#   ./scripts/vendor/ensure_llama_vendor_patches.sh /path/to/vendor/llama-cpp-c84b3020
set -euo pipefail
...
apply_vendor_patches() {
  ...
  if [[ -x "${ROOT}/scripts/vendor/apply_llama_vendor_patches.sh" ]]; then
  ...
  "${ROOT}/scripts/vendor/stage_llama_compat_for_vendor.sh" "${VENDOR}"
  "${ROOT}/scripts/vendor/stage_llama_ext_b8_for_vendor.sh" "${VENDOR}"
