...
if [[ ! -d "${VENDOR}/.git" ]]; then
  ...
  echo "  ./scripts/vendor/rebase_vendor_unified.sh" >&2
...
cd "${ROOT}"
# Prefer -mod=readonly so go generate does not hang on a full module VCS fetch
# (GOFLAGS=-mod=mod previously triggered git fetch of github.com/ollama/ollama).
if ! GOFLAGS=-mod=readonly go generate ./ml/backend/ggml/ggml/src; then
  echo ">>> go generate failed; regenerating ggml-metal-embed.metal via sed" >&2
  (
    cd "${ROOT}/ml/backend/ggml/ggml/src/ggml-metal"
    if [[ -f ggml-metal.metal ]]; then
      { echo "// Code generated by 'go generate'. DO NOT EDIT.";
        sed -e '/__embed_ggml-common.h__/r ../ggml-common.h' \
            ...
            -e '/#include "ggml-metal-impl.h"/r ggml-metal-impl.h' \
            -e '/#include "ggml-metal-impl.h"/d' \
            ggml-metal.metal; } >ggml-metal-embed.metal
    else
      echo ">>> skip metal embed regen: ggml-metal.metal gone (b10615 split kernels/*.metal)" >&2
    fi
  )
fi
