...
#
# Usage:
#   ./scripts/vendor/apply_llama_vendor_patches.sh
#   ./scripts/vendor/apply_llama_vendor_patches.sh /path/to/vendor/llama-cpp-c84b3020
set -euo pipefail
...
# True when every llama/patches/*.patch subject is already a commit on the pin.
# WHY: build scripts stage ANE/Metal hooks into a dirty vendor tree, then re-run
# apply; git am -3 against a dirty already-patched tree conflicts (e.g. 0009).
_vendor_has_all_patch_subjects() {
  local patch base subj
  local missing=0
  for patch in "${PATCH_DIR}"/*.patch; do
    ...
    # Strip RFC2047 leftovers / trailing junk from truncated subjects
    subj="${subj%%$'\r'}"
    if [[ -z "${subj}" ]]; then
      missing=$((missing + 1))
      continue
    fi
    if ! _vendor_has_patch_subject "${subj}"; then
      missing=$((missing + 1))
    fi
  done
  [[ "${missing}" -eq 0 ]]
...
_vendor_head_matches_pin_file() {
  local expected actual
  [[ -f "${ROOT}/LLAMA_CPP_VENDOR_HEAD" ]] || return 1
  expected="$(tr -d '[:space:]' < "${ROOT}/LLAMA_CPP_VENDOR_HEAD")"
  [[ -n "${expected}" ]] || return 1
  actual="$("${GIT_AM[@]}" rev-parse HEAD)"
  [[ "${actual}" == "${expected}" ]]
}
...
_skip_am_already_complete() {
  local count="$1"
  echo ">>> all patches already on vendor — skip git am (avoids dirty-tree 3-way conflicts)"
  for patch in "${PATCH_DIR}"/*.patch; do
    [[ -f "${patch}" ]] || continue
    touch "${PATCH_DIR}/.$(basename "${patch}" .patch).patched"
  done
  "${ROOT}/scripts/vendor/stage_llama_compat_for_vendor.sh" "${VENDOR}" 2>/dev/null || true
  "${ROOT}/scripts/vendor/stage_llama_ext_b8_for_vendor.sh" "${VENDOR}" 2>/dev/null || true
  "${ROOT}/scripts/vendor/stage_llama_kv_ext_for_vendor.sh" "${VENDOR}" 2>/dev/null || true
  echo ">>> OK: vendor +${count} commits (applied 0, skipped all — already complete)"
  exit 0
}

echo ">>> vendor checkout ${FETCH_REF:0:12}"
# Do NOT shallow-fetch the pin on an already-patched vendor — it can break
# FETCH_REF..HEAD ancestry used by subject checks. Fresh clones fetch in rebase.
...
if _vendor_is_patched; then
  ...
  # Abort any leftover am from a prior failed build before deciding.
  "${GIT_AM[@]}" am --abort 2>/dev/null || true
  rm -rf "${VENDOR}/.git/rebase-apply"
  patch_n="$(_patch_file_count)"
  if _vendor_head_matches_pin_file; then
    _skip_am_already_complete "${existing_count}"
  fi
  if [[ "${existing_count}" -ge "${patch_n}" ]]; then
    _skip_am_already_complete "${existing_count}"
  fi
  if _vendor_has_all_patch_subjects; then
    _skip_am_already_complete "${existing_count}"
  fi
else
