...
if [[ ! -f "${LLAMA_CPP}/CMakeLists.txt" ]]; then
  echo "error: llama.cpp missing at ${LLAMA_CPP} — run ./scripts/vendor/rebase_vendor_unified.sh --sync" >&2
  exit 1
fi
...
"${ROOT}/scripts/vendor/apply_llama_vendor_patches.sh" "${LLAMA_CPP}"
...
podman run --rm \
  ...
  bash -lc '
    ...
    ZEROLLAMA_COMPAT_SRC=/zerollama/llama/compat ZEROLLAMA_COMPAT_COPY=1 \
      ./scripts/vendor/stage_llama_compat_for_vendor.sh /llama.cpp
    ./scripts/vendor/stage_llama_ext_b8_for_vendor.sh /llama.cpp
    ./scripts/vendor/stage_llama_kv_ext_for_vendor.sh /llama.cpp
    set +e
