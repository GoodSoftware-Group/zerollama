...
#
# Usage:
#   ./scripts/install_cuda_stack.sh
#   SKIP_LLAMA_BUILD=1 ./scripts/install_cuda_stack.sh   # install only (binary already built)
#
# Steps:
...
if [[ "${SKIP_LLAMA_BUILD:-0}" != "1" ]] && [[ -f "${SIBLING}/ggml/src/ggml-cuda/fused-attn.cu" ]]; then
  ...
  "${ROOT}/scripts/create_cuda_fork_patch.sh"
...
if [[ "${SKIP_LLAMA_BUILD:-0}" != "1" ]]; then
  ...
  LLAMA_CPP_ROOT="${VENDOR}" ELIZA_LLAMA_SIBLING="${SIBLING}" \
    "${ROOT}/scripts/build_llama_server_container.sh"
else
...
_run_as_root env LLAMA_CPP_ROOT="${VENDOR}" "${ROOT}/scripts/install_cuda_llama_server.sh"

echo ">>> [4/4] deploy Python runtime"
_run_as_root "${ROOT}/scripts/deploy_runtime.sh"
