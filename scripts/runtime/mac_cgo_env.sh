...
#
#   source ./scripts/runtime/mac_cgo_env.sh && mac_cgo_env
...
mac_cgo_env_warn_path() {
  ...
  if [[ -n "$xclang" && -n "$first" && "$first" != "$xclang" ]]; then
    echo "warn: PATH clang is ${first} (not Xcode). Use ./scripts/build/build_zerollama_mac.sh or: eval \"\$(./scripts/runtime/mac_cgo_env.sh --export)\"" >&2
  fi
}
