...
#   METAL_SELF_START=1 M3_LLAMA_MODEL=/path/to/small.gguf ./scripts/phase11_13_15_metal_signoff.sh
#
# Serve already up:
#   ./scripts/phase11_13_15_metal_signoff.sh
#
# Skip Phase 15 GPU steps (CPU KV CI only):
#   PHASE15_LIVE=0 ./scripts/phase11_13_15_metal_signoff.sh
set -euo pipefail
...
if [[ "${METAL_SELF_START:-0}" == "1" ]]; then
  ...
  if [[ ! -f "${LLAMA_CPP_LIB:-}" ]]; then
    echo "METAL_SELF_START=1 needs ${LLAMA_CPP_LIB:-<unset>}; run ./scripts/build_llama_server.sh" >&2
    exit 1
  fi
...
"${ROOT}/scripts/phase11_metal_admission_smoke.sh"

echo ""
echo "== Phase 13 (Mac VRAM estimate) =="
"${ROOT}/scripts/phase13_metal_vram_smoke.sh"
