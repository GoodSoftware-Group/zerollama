#!/usr/bin/env bash
# Thin wrapper for training run_script → video-cli (Pure-C video-c).
# Emits PROGRESS: lines for the job queue.
# Prefer VIDEO_CLI; WAN_CLI remains for back-compat.
set -euo pipefail

CLI="${VIDEO_CLI:-${WAN_CLI:-}}"
if [[ -z "$CLI" || ! -x "$CLI" ]]; then
  echo "ERROR: VIDEO_CLI / WAN_CLI not set or not executable" >&2
  exit 1
fi
...
echo "PROGRESS: 0 starting video-cli"
