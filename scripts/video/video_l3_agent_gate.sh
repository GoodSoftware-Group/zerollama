...
#
# Usage:
#   ./scripts/video/video_l3_agent_gate.sh
#
# Optional L3 wiring (subprocess + GGUF; no VLM required):
#   RUN_E2E_L3=1 CUDA_LLAMA_MODEL=/path/to/model.gguf ./scripts/video/video_l3_agent_gate.sh
#
...
"${ROOT}/scripts/video/video_agent_cache_smoke.sh"
...
if [[ "${RUN_E2E_L3:-0}" == "1" ]]; then
  ...
  "${ROOT}/scripts/phase/l3_gate_report.sh" "${L3_OUT}"
fi

if [[ "${RUN_E2E_VIDEO_AGENT_INFER:-0}" == "1" ]]; then
  echo "== live video + inference cached_tokens gate =="
  "${ROOT}/scripts/video/video_agent_infer_smoke.sh"
fi
