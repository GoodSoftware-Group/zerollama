#!/usr/bin/env bash
# Install / build Pure-C Wan T2V client (wan-cli) and convert weights to GGUF.
#
...
WAN_C="$ROOT/x/wan-c"
OUT_ROOT="${WAN_C_ROOT:-$HOME/.zerollama/third_party/wan-c}"
CKPT="${1:-}"
if [[ "${1:-}" == "--ckpt-dir" ]]; then
  CKPT="${2:-}"
fi
...
if command -v uma >/dev/null 2>&1; then
  ...
  echo "ERROR: /tmp/uma_daemon.sock missing — start uma_daemon before wan-cli." >&2
  exit 1
fi
...
mkdir -p "$OUT_ROOT"
cp -f "$WAN_C/wan-cli" "$OUT_ROOT/wan-cli"
chmod +x "$OUT_ROOT/wan-cli"

if [[ -d "$CKPT" ]]; then
  echo "==> convert GGUF from $CKPT"
  python3 "$WAN_C/tools/convert_wan_to_gguf.py" --ckpt-dir "$CKPT" \
  ...
  if [[ -n "${SPM:-}" && -f "$SPM" ]]; then
    ...
    python3 "$WAN_C/tools/export_umt5_spm.py" "$SPM" -o "$OUT_ROOT/umt5.vocab" || true
  fi
else
...
export WAN_CLI="$OUT_ROOT/wan-cli"
export WAN_C_CKPT_DIR="$CKPT"
export WAN_C_GGUF="$OUT_ROOT/wan_t2v_1.3b.gguf"
export WAN_C_VOCAB="$OUT_ROOT/umt5.vocab"
EOF

echo "Installed wan-cli → $OUT_ROOT"
echo "Source: source $OUT_ROOT/env.sh"
echo "Then set ZEROLLAMA_WAN_CLI=\$WAN_CLI (or backend_paths.wan_cli) for POST /v1/videos"
