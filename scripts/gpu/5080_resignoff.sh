...
#   ./scripts/gpu/5080_resignoff.sh                 # all tiers (stop on first FAIL)
#   ./scripts/gpu/5080_resignoff.sh --tier 2        # L1 + L3 only
#   ./scripts/gpu/5080_resignoff.sh --radix         # append Radix live after tier 2
#   ./scripts/gpu/5080_resignoff.sh --build         # rebuild zerollama + sibling llama-server first
#   ./scripts/gpu/5080_resignoff.sh --vendor        # rebuild vendor llama-server (Radix seq-copy)
#   ./scripts/gpu/5080_resignoff.sh --no-serve      # assume serve already up (tier 1–2)
#
...
# shellcheck source=scripts/gpu/5080_env.sh
source "${ROOT}/scripts/gpu/5080_env.sh"
...
if run_tier 0; then
  ...
  "${ROOT}/scripts/phase/phase17_l2_pin_status.sh"
...
if run_tier 1; then
  ...
  5080_cd_repo
  "${ROOT}/scripts/gpu/gpu_5080_session.sh"
fi
...
if run_tier 2; then
  ...
  if [[ "${DO_RADIX}" -eq 1 ]]; then
    ...
    L3_RADIX_LIVE=1 L3_RADIX_OUT="${L3_RADIX_OUT}" CUDA_LLAMA_MODEL="${CUDA_LLAMA_MODEL}" \
      "${ROOT}/scripts/phase/l3_radix_prefix_smoke.sh"
  fi
fi
...
if run_tier 4; then
  ...
  LLAMA_SERVER_BIN="${LLAMA_SERVER_BIN}" P17_MODEL="${P17_MODEL}" \
    "${ROOT}/scripts/phase/phase17_linux_auto_smoke.sh"
  P17_NUM_PREDICT=32 LLAMA_SERVER_BIN="${LLAMA_SERVER_BIN}" P16_MODEL="${P17_MODEL}" \
    "${ROOT}/scripts/phase/phase16_edge_smoke.sh"
fi

if run_tier 5; then
  echo "== tier 5: optional periodic (skipped by default — run manually) =="
  echo "  L2: ./scripts/phase/l2_cuda_full_gate.sh"
  echo "  Radix: ./scripts/gpu/5080_resignoff.sh --tier 2 --radix --vendor"
  echo "  Training: RUN_E2E_TRAINING_OPS=1 ./scripts/gpu/gpu_5080_session.sh"
fi
