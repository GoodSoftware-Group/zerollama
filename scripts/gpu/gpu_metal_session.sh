...
#   METAL_SELF_START=1 ./scripts/gpu/gpu_metal_session.sh
#
# Prerequisite — serve already up (default):
#   ./scripts/serve/serve_mac_runtime.sh   # or zerollama serve + sidecar
#
# Minimal (coordination + /health metal fields):
#   ./scripts/gpu/gpu_metal_session.sh
#
# With runtime infer + snapshot (small GGUF):
#   export M3_LLAMA_MODEL=/path/to/small.gguf
#   METAL_SELF_START=1 RUN_E2E_PHASE14=1 ./scripts/gpu/gpu_metal_session.sh
#
# Optional:
...
if [[ "${METAL_SELF_START:-0}" == "1" ]]; then
  ...
  if [[ ! -f "${LLAMA_CPP_LIB}" ]]; then
    echo "METAL_SELF_START=1 needs ${LLAMA_CPP_LIB}; run ./scripts/build/build_llama_server.sh" >&2
    exit 1
  fi
