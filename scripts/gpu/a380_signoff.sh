...
#   ./scripts/gpu/a380_signoff.sh --tier 1
#
...
if run_tier 0; then
  ...
  if [[ -n "${LLAMA_SERVER_BIN:-}" ]]; then
    ...
    echo "  ./scripts/build/build_zerollama_a380.sh && sudo ./scripts/gpu/install_a380_llama_server.sh" >&2
    exit 1
  fi
...
if run_tier 2; then
  ...
  "${ROOT}/scripts/gpu/a380_vulkan_smoke.sh"
fi
