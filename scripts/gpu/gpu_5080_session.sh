...
# CT 1564: source scripts/gpu/5080_env.sh once per shell (paths, PYTHONPATH, RUN_E2E_PREFLIGHT=0).
# When resignoff or the operator already sourced it, honor those exports.
if [[ -n "${Z5080_ENV_LOADED:-}" ]]; then
  :
elif [[ -f "${ROOT}/scripts/gpu/5080_env.sh" && "${Z5080_AUTO_ENV:-0}" == "1" ]]; then
...
if [[ "${RUN_E2E_UPSTREAM_GGUF:-0}" == "1" ]]; then
  ...
  if ! declare -F 5080_start_serve >/dev/null 2>&1; then
    if [[ -f "${ROOT}/scripts/gpu/5080_env.sh" ]]; then
      # shellcheck source=scripts/gpu/5080_env.sh
      source "${ROOT}/scripts/gpu/5080_env.sh"
    fi
  fi
  if declare -F 5080_start_serve >/dev/null 2>&1; then
    ...
    echo "warn: RUN_E2E_UPSTREAM_GGUF=1 — source scripts/gpu/5080_env.sh so base smokes use profile-off serve" >&2
  fi
fi
...
if [[ "${RUN_E2E_PHASE14_SIGNOFF:-0}" == "1" ]]; then
  ...
  "${ROOT}/scripts/phase/phase14_5080_signoff.sh"
  ...
  if [[ "${RUN_E2E_INPROCESS:-0}" == "1" && -z "${RUN_E2E_LLAMA_BACKEND_SOURCE:-}" ]]; then
    ...
    env "${phase14_env[@]}" "${ROOT}/scripts/phase/phase14_backend_smoke.sh"
  fi
fi
...
if [[ "${RUN_E2E_L1:-0}" == "1" ]]; then
  ...
  CUDA_LLAMA_MODEL="${CUDA_LLAMA_MODEL}" "${ROOT}/scripts/phase/l1_cuda_full_gate.sh"
fi
...
if [[ "${RUN_E2E_L3:-0}" == "1" ]]; then
  ...
  CUDA_LLAMA_MODEL="${CUDA_LLAMA_MODEL}" "${ROOT}/scripts/phase/l3_cuda_full_gate.sh"
fi
