...
#   ./scripts/gpu/5080_resignoff.sh              # full re-sign-off
#   ./scripts/gpu/gpu_5080_session.sh            # tier 1 only
#
...
source "${Z5080_ROOT}/scripts/gpu/cuda_common_env.sh"
...
export Z5080_VENDOR_PIN="${Z5080_VENDOR_PIN:-f95de977}"
...
source "${Z5080_REPO}/scripts/training/training_uv_venv.sh"
...
5080_setup_venvs() {
  ...
  if [[ ! -d runtime/.venv ]]; then
    RUNTIME_UV_SYNC=1 "${Z5080_REPO}/scripts/runtime/runtime_uv_venv.sh"
  fi
  if [[ "${OLLAMA_TRAINING:-false}" == "true" ]]; then
    ...
    if [[ ! -d "$site" ]]; then
      TRAINING_UV_VENV="${Z5080_REPO}/.venv-training" TRAINING_UV_PYTHON_VER="${embed_py}" \
        "${Z5080_REPO}/scripts/training/training_uv_venv.sh" --verify
    fi
  fi
...
5080_build_zerollama() {
  ...
  if [[ -f "${Z5080_REPO}/scripts/training/training_embed_build_env.sh" ]] && pkg-config --exists python-3.11-embed 2>/dev/null; then
    # shellcheck source=scripts/training/training_embed_build_env.sh disable=SC1091
    source "${Z5080_REPO}/scripts/training/training_embed_build_env.sh" 3.11
  fi
...
5080_build_sibling_llama_server() {
  ...
  if [[ ! -x "${LLAMA_SERVER_BIN}" ]]; then
    LLAMA_CPP_ROOT="${Z5080_LLAMA_CPP}" "${Z5080_REPO}/scripts/build/build_llama_server.sh"
  fi
}
...
5080_build_vendor_llama_server() {
  ...
  LLAMA_CPP_ROOT="${Z5080_VENDOR_ROOT}" "${Z5080_REPO}/scripts/build/build_llama_server.sh"
  export LLAMA_SERVER_BIN="${Z5080_VENDOR_ROOT}/build/bin/llama-server"
  export LLAMA_CPP_BIN="$(dirname "${LLAMA_SERVER_BIN}")"
  export LLAMA_CPP_ROOT="${Z5080_VENDOR_ROOT}"
  export LD_LIBRARY_PATH="${LLAMA_CPP_BIN}:${LD_LIBRARY_PATH}"
  "${Z5080_REPO}/scripts/vendor/llama_patch_doctor.sh"
}
...
5080_print_env() {
  cat <<EOF
5080 env (source: scripts/gpu/5080_env.sh)
