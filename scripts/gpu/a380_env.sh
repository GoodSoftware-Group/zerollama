...
# shellcheck source=scripts/gpu/a380_llama_vendor.sh disable=SC1091
source "${ZA380_ROOT}/scripts/gpu/a380_llama_vendor.sh"
...
a380_start_serve() {
  a380_stop_serve
  a380_cd_repo
  # shellcheck source=scripts/serve/serve_a380_example.sh disable=SC1091
  bash "${ZA380_ROOT}/scripts/serve/serve_a380_example.sh" >> /tmp/zerollama-a380-serve.log 2>&1 &
...
a380_build_zerollama() {
  a380_cd_repo
  bash "${ZA380_ROOT}/scripts/build/build_zerollama_a380.sh"
}

a380_build_llama_vendor() {
  a380_cd_repo
  bash "${ZA380_ROOT}/scripts/build/build_zerollama_a380.sh" --llama-only
}
...
a380_print_env() {
  cat <<EOF
a380 env (source: scripts/gpu/a380_env.sh)
