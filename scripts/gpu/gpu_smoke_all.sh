...
#   RUN_E2E_PREFLIGHT=1  # run ./scripts/phase/phase12_golden_ci.sh before GPU steps (default in gpu_5080_session.sh)
...
#   ZEROLLAMA_RUNTIME_LLAMA_BACKEND=inprocess|llama-cpp-python ./scripts/phase/phase14_backend_smoke.sh
set -euo pipefail
...
if [[ "${RUN_E2E_TRAINING_OPS:-0}" == "1" ]]; then
  ...
  env "${train_env[@]}" "${ROOT}/scripts/e2e/e2e_training_ops_smoke.sh"
fi
...
# shellcheck disable=SC2086
env "${e2e_env[@]}" "${ROOT}/scripts/e2e/e2e_runtime_smoke.sh"

if [[ "${RUN_E2E_LEGACY:-0}" == "1" ]]; then
  echo "== legacy ggml smoke (after runtime; uses :8080 ggml runner) =="
  runtime_resume_if_needed
  # shellcheck disable=SC2086
  env RUN_E2E_LEGACY=1 RUN_E2E_LEGACY_ONLY=1 "${ROOT}/scripts/e2e/e2e_runtime_smoke.sh"
fi

echo "== GPU health report =="
runtime_resume_if_needed
"${ROOT}/scripts/gpu/gpu_health_report.sh"
...
if [[ "${RUN_E2E_PHASE13_SNAPSHOT:-0}" == "1" ]]; then
  ...
  GPU_PHASE13_SNAPSHOT_OUT="$snap_out" "${ROOT}/scripts/gpu/gpu_phase13_snapshot.sh" "${snap_args[@]}"
fi
