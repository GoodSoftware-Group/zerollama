...
#
# Usage:
#   ./scripts/gpu/gpu_lane_session.sh
#   CUDA_LANE=dual_4090 ./scripts/gpu/gpu_lane_session.sh
#   RUN_E2E_L1=1 RUN_E2E_L3=1 ./scripts/gpu/gpu_lane_session.sh   # 5080 lane only today
#
...
case "${LANE}" in
  dual_4090)
    # shellcheck source=scripts/gpu/dual_4090_env.sh disable=SC1091
    source "${ROOT}/scripts/gpu/dual_4090_env.sh"
    dual_4090_assert_topology
    echo "== CUDA common gate =="
    RUN_E2E_GPU="${RUN_E2E_GPU:-1}" RUN_E2E_PROXY="${RUN_E2E_PROXY:-1}" \
      "${ROOT}/scripts/gpu/cuda_common_gate.sh"
    ...
    echo "  CUDA_LANE=dual_4090 source scripts/gpu/dual_4090_env.sh"
    echo "  CUDA_LLAMA_MODEL=/path/to/9b.gguf ./scripts/phase/l1_cuda_full_gate.sh"
    echo "  CUDA_LLAMA_MODEL=/path/to/9b.gguf ./scripts/phase/l3_cuda_full_gate.sh"
    ;;
  rtx_5080|single_4090|single_cuda)
    if [[ -f "${ROOT}/scripts/gpu/5080_env.sh" ]]; then
      ...
      RUN_E2E_GPU="${RUN_E2E_GPU:-1}" "${ROOT}/scripts/gpu/cuda_common_gate.sh"
    fi
    ;;
  *)
