...
#
# Uses patched vendor/ (elizaOS + Ollama patches). Prefer ./scripts/build/build_llama_server.sh
...
#   BUILD_ZEROLLAMA_GO=1 ./scripts/build/build_ollama_llama_server_darwin.sh
set -euo pipefail
...
# shellcheck source=scripts/vendor/llama_unified_vendor_env.sh
source "${ROOT}/scripts/vendor/llama_unified_vendor_env.sh"
mac_cgo_env
...
if [[ "${BUILD_GO}" == "1" ]]; then
  ...
  "${ROOT}/scripts/build/build_zerollama_mac.sh" "${ROOT}/zerollama"
fi
