...
#
#   ./scripts/build/build_zerollama_a380.sh           # go + vendor llama (full stack)
#   ./scripts/build/build_zerollama_a380.sh --go-only # zerollama binary only
#   ./scripts/build/build_zerollama_a380.sh --llama-only
#
# After build: sudo ./scripts/gpu/install_a380_llama_server.sh && sudo systemctl restart zerollama
set -euo pipefail
...
if [[ "${DO_LLAMA}" -eq 1 ]]; then
  if [[ ! -d "${VENDOR}/.git" ]]; then
    ...
    "${ROOT}/scripts/vendor/rebase_vendor_unified.sh" --apply
  ...
  GGML_VULKAN=1 GGML_CUDA=OFF LLAMA_BUILD_WEBUI=OFF "${ROOT}/scripts/build/build_llama_server.sh"

  # shellcheck source=scripts/gpu/a380_llama_vendor.sh
  source "${ROOT}/scripts/gpu/a380_llama_vendor.sh"
...
echo ">>>       sudo ./scripts/gpu/install_a380_llama_server.sh"
