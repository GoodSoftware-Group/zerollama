...
source "${_ZEROLLAMA_ROOT}/scripts/vendor/ensure_llama_cpp_sibling.sh"

if [[ ! -f "${ROOT}/CMakeLists.txt" ]]; then
  if [[ "${ROOT}" == "${_VENDOR_ROOT}" ]]; then
    echo "patched vendor missing at ${ROOT}; run ./scripts/vendor/rebase_vendor_unified.sh --sync" >&2
    exit 1
  fi
...
if _is_vendor_root "${ROOT}"; then
  if [[ "${ZEROLLAMA_SKIP_VENDOR_APPLY:-0}" != "1" ]]; then
    "${_ZEROLLAMA_ROOT}/scripts/vendor/apply_llama_vendor_patches.sh" "${ROOT}"
  fi
fi
...
if [[ "$(uname -s)" == Darwin && "${ZEROLLAMA_SKIP_ANE_HOOK_SYNC:-0}" != "1" ]]; then
  if ! _is_vendor_root "${ROOT}"; then
    if [[ -f "${_ZEROLLAMA_ROOT}/scripts/vendor/sync_ane_hook_to_llama_cpp.sh" ]]; then
      echo ">>> sync ANE draft hook → ${ROOT}" >&2
      LLAMA_CPP_ROOT="${ROOT}" "${_ZEROLLAMA_ROOT}/scripts/vendor/sync_ane_hook_to_llama_cpp.sh"
    fi
  fi
fi
...
_probe_ollama_compat_loader() {
  ...
  if grep -q 'llama_ollama_compat::translate_metadata' "${vendor_root}/src/llama-model-loader.cpp"; then
    ...
    echo "error: vendor loader missing Ollama compat hooks — run ./scripts/vendor/ensure_llama_vendor_patches.sh" >&2
    exit 1
  fi
}
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  if [[ -f "${ROOT}/common/ane_draft_hook.cpp" ]]; then
    bash "${_ZEROLLAMA_ROOT}/scripts/vendor/patch_vendor_linux_ane_hook.sh" "${ROOT}"
  fi
  if [[ -f "${_ZEROLLAMA_ROOT}/llama/llama.cpp/common/ane_draft_hook.cpp" ]]; then
    bash "${_ZEROLLAMA_ROOT}/scripts/vendor/patch_vendor_linux_ane_hook.sh" "${_ZEROLLAMA_ROOT}/llama/llama.cpp"
  fi
fi
...
_probe_seq_copy_route() {
  ...
  local root="$2"
  local has_route=0
  # WHY grep -a (not `strings | grep -q`): under `set -o pipefail`, grep -q
  # closes the pipe as soon as it finds a match, which can make an earlier
  # `strings` in the pipeline die from SIGPIPE (exit 141) — pipefail then
  # reports the whole pipeline as failed even though the match was found.
  ...
  if [[ "${has_route}" -eq 1 ]]; then
    ...
    echo "  ./scripts/vendor/rebase_vendor_unified.sh --apply --sync && ./scripts/build/build_llama_server.sh" >&2
    exit 1
  fi
}
...
if [[ "$(uname -s)" == "Darwin" ]]; then
  ...
  source "${ZEROLLAMA_ROOT}/scripts/runtime/mac_cgo_env.sh"
  mac_cgo_env
