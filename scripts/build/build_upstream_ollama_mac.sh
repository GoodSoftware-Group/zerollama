...
#   OLLAMA_UPSTREAM_DIR=../ollama-upstream ./scripts/build/build_upstream_ollama_mac.sh
#   BUILD_UPSTREAM_GO=0 ./scripts/build/build_upstream_ollama_mac.sh   # llama-server only
set -euo pipefail
...
if [[ ! -d "${UPSTREAM}/.git" ]]; then
  echo ">>> missing ${UPSTREAM}; run ./scripts/gpu/clone_upstream_ollama.sh" >&2
  exit 1
fi
