...
#   LLAMA_SERVER_BIN=/path/to/llama-server ./scripts/serve/serve_linux_auto.sh
set -euo pipefail
...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "serve_linux_auto is Linux-only; on Darwin use ./scripts/serve/serve_llama_server_backend.sh or default ggml serve" >&2
  exit 1
fi
