...
// skipFinalNorm — when set, Forward returns pre-final-RMSNorm hidden so
// GRAPH token-tail (F0700 NORM_MUL_F16→GEMV→ARGMAX) owns the final norm.
var skipFinalNorm atomic.Bool

// SetSkipFinalNorm controls whether Model.Forward omits the final RMSNorm.
func SetSkipFinalNorm(v bool) { skipFinalNorm.Store(v) }

// SkipFinalNorm reports the current skip-final-norm flag.
func SkipFinalNorm() bool { return skipFinalNorm.Load() }
...
type Config struct {
	...
	// Optional mlx-lm quantization block (bits/group_size), from text_config
	// or the VL envelope. Populated by parseConfig into QuantBits/GroupSize.
	Quantization *struct {
...
func parseConfig(configData []byte) (Config, error) {
	...
	// VL envelopes often keep quantization on the outer config while text
	// dims live in text_config. Prefer nested, then top-level.
	if cfg.Quantization == nil {
		...
		_ = json.Unmarshal(configData, &outer)
		cfg.Quantization = outer.Quantization
	}
	if cfg.Quantization != nil {
		if cfg.Quantization.Bits > 0 {
			cfg.QuantBits = cfg.Quantization.Bits
		}
		if cfg.Quantization.GroupSize > 0 {
			cfg.QuantGroupSize = cfg.Quantization.GroupSize
		}
		if cfg.QuantBits > 0 {
...
func rawTextConfig(configData []byte) ([]byte, bool) {
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(configData, &raw); err != nil {
		return nil, false
	}
	textRaw, ok := raw["text_config"]
	if !ok || len(textRaw) == 0 {
		return nil, false
	}
	return textRaw, true
...
func NewModel(root *model.Root) (base.Model, error) {
	...
	// Prefer config.json quantization over blob-inferred QuantType.
	// 2-bit/gs128 and 4-bit/gs64 share packed shapes; guessing INT4 from
	// blobs alone mis-dequantizes bonsai-class models (hidden 5120 → 2560).
	if cfg.QuantBits > 0 {
		if cfg.QuantGroupSize <= 0 {
			if cfg.QuantBits == 2 {
				cfg.QuantGroupSize, _, _ = model.QuantizationParams("INT2")
			} else {
	...
	// OptIQ / mlx-lm often put per-tensor bits in config.json quantization{}
	// (blob headers empty). Merge after blob scan so config wins on conflicts.
	...
	if textRaw, ok := rawTextConfig(configData); ok {
		model.ApplyQuantizationFromConfig(textRaw, &qfields)
	}
...
func supportsGatherQMM(mode string, bits int) bool {
	switch mode {
	case "affine":
		// Match gptoss / MLX gather_qmm: 3/4/6/8-bit affine experts stay
		// packed. Missing 6 here forced full MoE dequant on coder-next:6bit
		// and OOM-killed the mlx runner (see serve log "no gather kernel").
