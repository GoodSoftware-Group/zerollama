// Package granite provides IBM Granite dense decoder-only transformers for MLX.
//
// Granite is Llama-shaped but applies four config scalars from the HF / mlx-lm
// GraniteForCausalLM path: embedding_multiplier, attention_multiplier,
// residual_multiplier, and logits_scaling.
package granite
...
func newModel(root *model.Root) (base.Model, error) {
	...
	if cfg.EmbeddingMultiplier == 0 {
		cfg.EmbeddingMultiplier = 1
	}
	if cfg.ResidualMultiplier == 0 {
		cfg.ResidualMultiplier = 1
	}
	if cfg.LogitsScaling == 0 {
		cfg.LogitsScaling = 1
	}
	// Granite uses attention_multiplier in place of 1/sqrt(head_dim).
...
func (m *Model) Forward(b *batch.Batch, caches []cache.Cache) *mlx.Array {
	...
	h := mlx.MulScalar(m.EmbedTokens.Forward(b.InputIDs), m.EmbeddingMultiplier)
...
func (m *Model) Unembed(x *mlx.Array) *mlx.Array {
	return mlx.DivScalar(m.LMHead.Forward(x), m.LogitsScaling)
}
...
func (l *Layer) Forward(x *mlx.Array, b *batch.Batch, c cache.Cache, positions *mlx.Array, B, L int32, cfg *Config) *mlx.Array {
	attn := l.Attention.Forward(l.AttentionNorm.Forward(x, cfg.RMSNormEps), b, c, positions, B, L, cfg)
	h := mlx.Add(x, mlx.MulScalar(attn, cfg.ResidualMultiplier))
	mlp := l.MLP.Forward(l.MLPNorm.Forward(h, cfg.RMSNormEps))
	return mlx.Add(h, mlx.MulScalar(mlp, cfg.ResidualMultiplier))
}
