...
int wan_pipeline_t2v(wan_ctx *ctx, const wan_gen_params *p, float *rgb_out,
  ...
  for (int step = 0; step < p->steps; step++) {
    ...
    memcpy(cond, latent, latent_n * sizeof(float));
    {
      ...
      if (wan_profile_on()) {
        double dt = wan_profile_now_ms() - t0;
        wan_profile_add_ms("dit_cond", dt);
        fprintf(stderr, "wan-c: step %d cond ms=%.0f\n", step + 1, dt);
      }
    }
    ...
    if (p->cfg_scale > 1.0001f) {
      ...
      memcpy(uncond, latent, latent_n * sizeof(float));
      {
        ...
        if (wan_profile_on()) {
          double dt = wan_profile_now_ms() - t0;
          wan_profile_add_ms("dit_uncond", dt);
          fprintf(stderr, "wan-c: step %d uncond ms=%.0f\n", step + 1, dt);
        }
      }
