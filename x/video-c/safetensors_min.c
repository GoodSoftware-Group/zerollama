...
static st_dtype_t parse_dtype(const char *s, size_t n) {
  ...
  if (n == 2 && strncmp(s, "I8", 2) == 0)
    return ST_DTYPE_I8;
  if (n == 2 && strncmp(s, "U8", 2) == 0)
    return ST_DTYPE_U8;
  return ST_DTYPE_UNKNOWN;
}

/* Convert src[0..n) of packed 16-bit floats to f32, parallelized across cores
 * for large tensors (small ones keep dispatch overhead away via threshold). */
static void convert16_parallel(const uint16_t *src, float *dst, size_t n,
                               int bf16) {
  const size_t kMinParallel = 1u << 20;
  if (n < kMinParallel) {
    if (bf16) {
  ...
  if (ncore < 2)
    ncore = 2;
  size_t per = (n + (size_t)ncore - 1) / (size_t)ncore;
...
const st_tensor_t *st_tensor_at(const st_file *sf, int index) {
  if (!sf || index < 0 || index >= sf->n_tensors)
    return NULL;
  return &sf->tensors[index];
}
...
const uint8_t *st_tensor_ptr(const st_file *sf, const st_tensor_t *t) {
  if (!sf || !t || t->nbytes == 0)
    return NULL;
  if (sf->data_off + t->data_end > sf->size)
    return NULL;
  return sf->map + sf->data_off + t->data_begin;
}
...
int st_tensor_to_f32(const st_file *sf, const st_tensor_t *t, float *dst,
  ...
  if (n > dst_n || t->nbytes == 0)
    return -1;
  const uint8_t *raw = st_tensor_ptr(sf, t);
  if (!raw)
    return -1;
