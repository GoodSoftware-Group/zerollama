/* Vendored from antirez/h3.c (MIT) — portable host layout/schedule for video-c H3. */
#include "h3_host.h"
...
int h3_rope_spatial_axis(int dim, int patch, double sqrt_area, double *out,
                         int out_n) {
  if (!out || dim < patch || patch < 1 || (dim % patch) != 0 || sqrt_area <= 0.0)
    return -1;
  int n = dim / patch;
  if (out_n < n)
    return -1;
  ...
  double step = ratio / (double)n;
  for (int i = 0; i < n; i++)
    out[i] = (left + (double)i * step) * H3_ROPE_SPATIAL_SCALE;
  return n;
}

int h3_rope_temporal_grid(int num_latent_frames, double origin, double *out,
                          int out_n) {
  if (!out || num_latent_frames < 1 || out_n < num_latent_frames)
    return -1;
  double t = origin;
  for (int i = 0; i < num_latent_frames; i++) {
    out[i] = t;
    t += h3_frame_rescale * (double)h3_frame_per_token[i % 5];
  }
  return num_latent_frames;
}

double h3_rope_temporal_span(int num_latent_frames) {
  if (num_latent_frames < 1)
    return 0.0;
  double sum = 0.0;
  for (int index = 0; index < num_latent_frames; index++)
    sum += h3_frame_rescale * (double)h3_frame_per_token[index % 5];
  return sum;
}
...
int h3_resolve_dit_pack_path(char *buf, size_t n) {
  if (!buf || n < 2)
    return 0;
...
int h3_resolve_qwen4b_dir(char *buf, size_t n) {
  if (!buf || n < 2)
    return 0;
  ...
  snprintf(buf, n, "%s/.zerollama/models/Qwen3-VL-4B-Instruct", home);
  return 1;
}
