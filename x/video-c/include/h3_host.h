...
/* RoPE packing grids (MLX packing.py / antirez h3_frame_grid). */
#define H3_ROPE_SPATIAL_SCALE 32.0
#define H3_ROPE_FRAME_RESCALE (5.0 / 3.0)
/* dim = latent H or W; patch usually 2; sqrt_area = sqrt(H*W). */
...
/* Host v matches unnegated DiT head. CONST denoised = x + σ·v
 * (Comfy: denoised = x − σ·(−head)). */
...
/* Generate weights: pruned ConvRot DiT (`H3_DIT_ST`) and Qwen3-VL-4B (`H3_QWEN_TE_DIR`). */
