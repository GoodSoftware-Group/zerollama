...
static int vae_resblock_same(wan_ctx *ctx, float *x, int C, int lt, int lh,
  ...
  /* W2/W6 (C*C*27 f32, 16MiB each) are needed only by the host fallback;
   * the broker path uses the banked copies (vae_tip_residual_bank). Load them
   * lazily in the fallback below to avoid 2x32MiB materialize per block. */
  float *W2 = NULL, *W6 = NULL;
  float *h = calloc(n, sizeof(float));
  float *y = calloc(n, sizeof(float));
  int ok = 0;
  size_t expect_w = (size_t)C * (size_t)C * 3 * 3 * 3;
  if (h && y && ng0 == (size_t)C && ng3 == (size_t)C && g0 && g3 &&
      wan_gguf_has(ctx, n2w) && wan_gguf_has(ctx, n6w)) {
    ...
    if (!ok) {
      W2 = wan_load_tensor_f32(ctx, n2w, &nw2);
      W6 = wan_load_tensor_f32(ctx, n6w, &nw6);
      if (!W2 || !W6 || nw2 != expect_w || nw6 != expect_w)
        goto done;
      memcpy(h, x, n * sizeof(float));
...
static int vae_resblock_expand(wan_ctx *ctx, float **feat, int *C, int lt,
  ...
  float *g3 = wan_load_tensor_f32(ctx, n3, &ng3);
  float *b6 = wan_load_tensor_f32(ctx, n6b, &nb6);
  /* W2/W6/Ws are needed only by the host fallback (broker uses bank). */
  float *W2 = NULL, *W6 = NULL, *Ws = NULL, *bs = NULL;
  ...
  if (h && y && out && h2 && g0 && g3 && ng0 == (size_t)Cin &&
      ng3 == (size_t)Cout && wan_gguf_has(ctx, n2w) && wan_gguf_has(ctx, nsw)) {
    int try_broker =
    ...
    if (!ok) {
      W2 = wan_load_tensor_f32(ctx, n2w, &nw2);
      W6 = wan_load_tensor_f32(ctx, n6w, &nw6);
      Ws = wan_load_tensor_f32(ctx, nsw, &nws);
      bs = wan_load_tensor_f32(ctx, nsb, &nbs);
      if (!W2 || !W6 || !Ws || nw2 != exp2 || nw6 != exp6 || nws != exps)
        goto done_exp;
      memcpy(h, *feat, nin * sizeof(float));
