...
func newGrammarEngine(logitsWidth int, tok *tokenizer.Tokenizer) *grammarEngine {
	library, err := mlx.LoadedLibraryPath()
	if err != nil {
		slog.Warn("Structured output is unavailable", "error", err)
		return nil
	}
	if err := validateGrammarVocab(logitsWidth, tok.VocabSize()); err != nil {
		slog.Warn("Structured output is unavailable", "error", err)
		return nil
	}
	pieces := make([]string, logitsWidth)
	for id := range logitsWidth {
		pieces[id] = tok.Decode([]int32{int32(id)})
	}
	stops := slices.DeleteFunc(slices.Clone(tok.EOSTokens()), func(id int32) bool {
...
func logitsWidth(m base.Model) (width int) {
	cacheFactory, ok := m.(interface{ NewCaches() []cache.Cache })
	if !ok {
		return 0
	}
	...
	return logits.Dim(logits.NumDims() - 1)
}
