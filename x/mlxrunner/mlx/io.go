...
func loadSafetensorsStream() C.mlx_stream {
	if runtime.GOOS == "darwin" {
		return mlxCheck(C.mlx_default_cpu_stream_new())
	}
	return mlxCheck(C.mlx_default_gpu_stream_new())
}
...
func SaveSafetensorsWithMetadata(path string, arrays map[string]*Array, metadata map[string]string) error {
	...
	cArrays := mlxCheck(C.mlx_map_string_to_array_new())
	...
	cMetadata := mlxCheck(C.mlx_map_string_to_string_new())
