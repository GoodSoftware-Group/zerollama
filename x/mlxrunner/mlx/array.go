...
func FromValues[S ~[]E, E arrayTypes](s S, shape ...int) *Array {
	...
	tt.ctx = mlxCheck(C.mlx_array_new_data(data, unsafe.SliceData(cShape), C.int(len(cShape)), C.mlx_dtype(dtype)))
	return tt
}
...
func (t *Array) Int() int {
	...
	case DTypeUint32:
		var item C.uint32_t
		mlxCheck(C.mlx_array_item_uint32(&item, t.ctx))
		return int(item)
	case DTypeInt64:
		var item C.int64_t
		mlxCheck(C.mlx_array_item_int64(&item, t.ctx))
		return int(item)
	case DTypeUint64:
		var item C.uint64_t
		mlxCheck(C.mlx_array_item_uint64(&item, t.ctx))
		return int(item)
	default:
...
func (t *Array) Float() float64 {
	...
	case DTypeFloat64:
		var item C.double
		mlxCheck(C.mlx_array_item_float64(&item, t.ctx))
		return float64(item)
	default:
...
func (t *Array) Ints() []int {
	...
	for i, f := range unsafe.Slice(mlxCheck(C.mlx_array_data_int32(t.ctx)), len(ints)) {
		ints[i] = int(f)
	}
	return ints
}
...
func (t *Array) Floats() []float32 {
	...
	for i, f := range unsafe.Slice(mlxCheck(C.mlx_array_data_float32(t.ctx)), len(floats)) {
		floats[i] = float32(f)
	}
	return floats
}
...
func (t *Array) Save(name string) error {
	...
	mlxCheck(C.mlx_save(cName, t.ctx))
	return nil
}
