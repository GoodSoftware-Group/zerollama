...
func TestBestRestorableOffsetPrefersSnapshotBoundary(t *testing.T) {
	root := &trieNode{lastUsed: time.Now()}
	n4 := &trieNode{tokens: []trieKey{1, 2, 3, 4}, endOffset: 4, parent: root, lastUsed: time.Now()}
	n8 := &trieNode{tokens: []trieKey{5, 6, 7, 8}, endOffset: 8, parent: n4, lastUsed: time.Now()}
	n10 := &trieNode{tokens: []trieKey{9, 10}, endOffset: 10, parent: n8, lastUsed: time.Now()}
...
func TestCapTrieMatchForRestorePartialEdge(t *testing.T) {
	root := &trieNode{lastUsed: time.Now()}
	parent := &trieNode{
		tokens:    make([]trieKey, 8192),
		endOffset: 8192,
		parent:    root,
		lastUsed:  time.Now(),
	}
	for i := range parent.tokens {
		parent.tokens[i] = trieKey(i + 1)
	}
	partial := &trieNode{
		tokens:    make([]trieKey, 8192),
		endOffset: 16384,
	...
	for i := range partial.tokens {
		partial.tokens[i] = trieKey(i + 8193)
	}
	root.children = []*trieNode{parent}
