...
struct llama_hparams {
    ...
    // Dual Chunk Attention (0 = disabled). chunk_len = dca_chunk_size - dca_local_size.
    uint32_t dca_chunk_size = 0;
    uint32_t dca_local_size = 0;
    uint32_t dca_orig_ctx   = 0; // original_max_position_embeddings for s(L)

    bool dca_enabled() const {
        return dca_chunk_size > dca_local_size && dca_local_size > 0;
    }

    uint32_t dca_chunk_len() const {
        return dca_enabled() ? (dca_chunk_size - dca_local_size) : 0;
    }
