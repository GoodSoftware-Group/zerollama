...
void llama_model_qwen3next::load_arch_hparams(llama_model_loader & ml) {
    ...
    // Mark recurrent layers (linear attention layers). MTP layers (the last
...
void llama_model_qwen3next::load_arch_tensors(llama_model_loader & ml) {
    ...
    auto load_block_mtp = [&](int il) {
        ...
        // MTP head is full attention + MoE FFN (never linear-attn / SSM).
...
llama_model_qwen3next::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    // Pre-norm hidden state: used by speculative MTP to seed the draft head.
    cb(cur, "h_pre_norm", -1);
...
// LLM_GRAPH_TYPE_DECODER_MTP draft head for Qwen3-Next (upstream #25589)
llama_model_qwen3next::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    : llm_graph_context(params) {
    GGML_ASSERT(hparams.nextn_predict_layers > 0 && "QWEN3NEXT MTP requires nextn_predict_layers > 0");
    GGML_ASSERT(hparams.nextn_predict_layers == 1 && "QWEN3NEXT MTP currently only supports a single MTP block");
    ...
    // Qwen3-Next: joint Q+gate projection (same layout as trunk full-attn layers)
