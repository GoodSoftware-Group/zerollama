...
struct llama_vocab {
    ...
    // Fast BPE id-pair table (built at load from merges + token_to_id). Empty => legacy string path.
    // WHY: see docs/faster-bpe-tokenize.md (M15d / patch 0106) — megaprompt merge without string keys.
    bool has_bpe_id_pairs() const;
    bool find_bpe_id_pair(llama_token left, llama_token right, int & rank, llama_token & merged) const;
    // UTF-8 piece (len<=4) -> token id for initial BPE symbols; NULL on miss.
    // WHY: avoid per-char std::string token_to_id (Qwen2 regress without this).
    llama_token find_bpe_piece4(const char * text, size_t len) const;
