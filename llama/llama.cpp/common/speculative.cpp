...
struct common_speculative_impl_draft_eagle3 : public common_speculative_impl {
        ...
        , params(params.draft)
    {
        ...
        if (const char * ane_draft = std::getenv("ZEROLLAMA_ANE_DRAFT")) {
            if (ane_draft[0] == '1' || ane_draft[0] == 'T' || ane_draft[0] == 't' ||
                ane_draft[0] == 'Y' || ane_draft[0] == 'y') {
                const int proxy_ch = std::min(512, std::max(64, (n_embd_dec + 7) / 8));
                LOG_INF("%s: ZEROLLAMA_ANE_DRAFT — ANE IOSurface handoff is lab-only (ggml_backend_dev_buffer_from_iosurface not wired in draft() yet)\n", __func__);
                LOG_INF("%s:   draft n_embd=%d encoder n_embd=%d ANE proxy conv ~%d x 16 — see docs/ane-ggml-iosurface-hook.md\n",
                        __func__, n_embd_dec, n_embd_enc, proxy_ch);
            }
        }
    }
