...
common_params common_base_params_to_speculative(const common_params & params) {
    ...
    // The draft context's n_outputs_max must be large enough for the largest batch we
    // will ever decode() on it. For plain draft_simple that's just n_parallel (one
    // token per sequence at a time), but the DFlash "milady" path
    // (common_ane_draft_sync_target_cross + common_speculative_impl_draft_simple::process)
    // mirrors the *entire* target batch onto the draft model to keep cross.v_embd in
    // sync, which can contain far more than n_parallel tokens needing outputs in a
    // single ubatch. Sizing this too small causes output_reserve() to hit
    // GGML_ASSERT(n_outputs_max <= cparams.n_outputs_max) and abort the whole process.
    // Use n_batch (same conservative bound the target context uses) to cover both cases.
