...
common_device_memory_data_vec common_get_device_memory_data(
    ...
    std::vector<llama_device_memory_data> impl = common_get_device_memory_data_impl(
            path_model, mparams, cparams, devs, hp_ngl, hp_n_ctx_train, hp_n_expert, hp_n_swa, log_level);
...
static void common_params_fit_impl(
        ...
        if (dmds_extra.empty() || n_ctx_extra != cparams->n_ctx) {
            ...
            try {
                measured = common_get_device_memory_data_impl(
                    extra->path_model, extra->mparams, extra->cparams, devs_extra, ngl_extra, nct_extra, nex_extra, nswa_extra, log_level);
    ...
    dmds_t dmds_full = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_nswa, log_level);
    ...
    if (n_ctx_auto) {
        ...
        if (n_streams > 1) {
            ...
            dmds_full = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_nswa, log_level);
        }
    }
        ...
        if (global_surplus < 0) {
            ...
            if (n_ctx_auto) {
                if (n_ctx_max > n_ctx_min_total) {
                    ...
                    dmds_t dmds_min_ctx = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_nswa, log_level);
...
void common_fit_print(
    ...
    auto dmd = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_nswa, GGML_LOG_LEVEL_ERROR);
