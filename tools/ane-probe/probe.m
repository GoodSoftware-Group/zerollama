// Minimal ANE smoke probe via maderix/ane libane_bridge.dylib.
//
// Why subprocess (not CGO in zerollama): private _ANEClient APIs break on macOS updates;
// this binary isolates compile/eval risk. Compiles a tiny 1x1 conv MIL program, runs eval,
// prints JSON on stdout for discover/ProbeANE. See docs/ane-probe.md.
//
// Build: make -C tools/ane-probe  (requires ANE_REPO with bridge built)
...
static NSData *buildWeightBlob(int ch) {
    ...
    for (NSUInteger j = 0; j < wsize / 2; j++) {
        fp16[j] = 0x3400; // ~0.25 fp16
    }
    return [NSData dataWithBytesNoCopy:buf length:total freeWhenDone:YES];
}

static void emitJSON(BOOL ok, const char *err, double evalMs, int compileCount) {
    NSMutableString *json = [NSMutableString stringWithFormat:
        @"{\"ok\":%@,\"eval_ms\":%.4f,\"compile_count\":%d,\"channels\":64,\"spatial\":16,\"source\":\"maderix/ane-bridge\"",
        ok ? @"true" : @"false", evalMs, compileCount];
    if (err && err[0]) {
...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        if (ane_bridge_init() != 0) {
            emitJSON(NO, "ane_bridge_init failed", 0, 0);
            return 1;
        }
        ...
        if (!milData || !wb) {
            emitJSON(NO, "MIL/weight allocation failed", 0, 0);
            return 1;
        }
        ...
        ANEKernelHandle *kernel = ane_bridge_compile_multi_weights(
            ...
            1, &ioBytes, 1, &ioBytes);
        if (!kernel) {
            emitJSON(NO, "ane_bridge_compile failed", 0, ane_bridge_get_compile_count());
            return 1;
        }
        ...
        if (!inBuf || !outBuf) {
            ...
            emitJSON(NO, "IO buffer allocation failed", 0, ane_bridge_get_compile_count());
            free(inBuf);
            free(outBuf);
            return 1;
        }
        ...
        for (int i = 0; i < 3; i++) {
            if (!ane_bridge_eval(kernel)) {
                ...
                free(inBuf);
                free(outBuf);
                emitJSON(NO, "ane_bridge_eval warmup failed", 0, ane_bridge_get_compile_count());
                return 1;
            }
        }
        ...
        for (int i = 0; i < iters; i++) {
            if (!ane_bridge_eval(kernel)) {
                ...
                free(inBuf);
                free(outBuf);
                emitJSON(NO, "ane_bridge_eval failed", 0, ane_bridge_get_compile_count());
                return 1;
            }
        }
        ...
        if (!finite) {
            emitJSON(NO, "non-finite output", evalMs, compileCount);
            return 1;
        }

        emitJSON(YES, NULL, evalMs, compileCount);
        return 0;
    }
}
