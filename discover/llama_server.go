...
// llama-server GPU discovery (Phase 17).
//
// Why this file exists: upstream schedulers read CUDA compiled archs, ROCm gfx
// targets, and free VRAM from llama-server startup output. The legacy ggml
// bootstrap subprocess never exposed the same fields consistently — leading to
// wrong layer splits or crashes on unsupported CUDA CC.
//
// Why hybrid bootstrap (see runner.go): Mac default remains ggml Metal; we only
// probe llama-server when Linux auto-default applies or the operator opts in
// with ZEROLLAMA_LLAMA_SERVER=1 or auto (Linux serve). Otherwise we fall back to ollama-engine /info.
