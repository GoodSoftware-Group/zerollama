...
func TestLlamaServerCompletionPaddedInjectUsesMultimodalPrompt(t *testing.T) {
	var capturedReq llamaServerCompletionRequest
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/detokenize":
			...
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
				t.Errorf("invalid detokenize body: %v", err)
				return
			}
			...
			fmt.Fprintf(w, `{"content":%q}`, b.String())
		case "/completion":
			if err := json.NewDecoder(r.Body).Decode(&capturedReq); err != nil {
				t.Errorf("invalid request body: %v", err)
				return
	...
	const mediaMarker = "<__ollama_media_padded__>"
	runner := &llamaServerRunner{
		port:        portInt,
		cmd:         fakeRunningCmd(),
		sem:         semaphore.NewWeighted(1),
	...
	opts := api.DefaultOptions()
	err := runner.Completion(t.Context(), CompletionRequest{
		Prompt: "ignored when prompt_tokens set",
		PromptTokens: []int{
			100, qwenVLVisionStart, 151655, qwenVLVisionEnd, 200,
		},
		PaddedLayoutConsume: PaddedLayoutConsumeQwen3VLHFRunner,
		Images:              []ImageData{{ID: 0, Data: []byte("frame-bytes")}},
		Options:             &opts,
	}, func(cr CompletionResponse) {})
	if err != nil {
		t.Fatalf("Completion error: %v", err)
	...
	if got, want := promptObj["prompt_string"], "100"+mediaMarker+"200"; got != want {
		t.Fatalf("prompt_string = %q, want %q", got, want)
	}
	data, ok := promptObj["multimodal_data"].([]any)
	if !ok || len(data) != 1 {
		t.Fatalf("multimodal_data = %v, want 1 entry", promptObj["multimodal_data"])
	}
	if got, want := data[0], base64.StdEncoding.EncodeToString([]byte("frame-bytes")); got != want {
		t.Fatalf("multimodal_data[0] = %q, want %q", got, want)
	}
}
...
func TestAppendMainGPUArgs(t *testing.T) {
	tests := []struct {
		...
		{
			name: "zero treated as unset in zerollama (int MainGPU)",
			opts: api.Options{Runner: api.Runner{MainGPU: 0}},
			want: []string{"base"},
		},
		{
			name: "explicit nonzero selects requested gpu",
			opts: api.Options{Runner: api.Runner{MainGPU: 1}},
...
func TestShouldRetryMMProjCPUOffloadArch(t *testing.T) {
