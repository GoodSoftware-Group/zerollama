...
**Fail-closed MTP expect (F0750):** `ZEROLLAMA_MLX_MTP=require` refuses runner load when the checkpoint has no draft/SelfDraft head (no silent AR demotion). Default unset remains soft AR (broken companions log and continue). Dense draft linears 4-bit at load (`ZEROLLAMA_MLX_DRAFT_QUANT=off` to keep dtype). **PLD** parks on novel prompts (MTP still runs), stacks in front of MTP/assistant when the prompt looks repetitive, and re-enables if the generated tail echoes the prompt. Sliding-window fused/prefill reads use `window+L-1` keys. Large idle trie snapshots pack to FP8. Fused verify at hd 192/256 splits Q so MLX stays on the vector SDPA kernel. Spec width is restored from `mlx-round-cost/` on Load. Triple n-gram loops stop as length. FIM/reserved tokenizer ids are not sampled (`ZEROLLAMA_MLX_SUPPRESS_RESERVED=0` to allow). Knob sheet: `zerollama doctor` (MLX knobs + last decode + round-cost); serve logs `mlx tune`; runner `GET /v1/status` includes `Knobs`/`Tune`. Cut-off replies: `continue_final_message` on `/v1/chat/completions` and `/api/chat`; `/v1/messages` infers it when the last turn is assistant text. `/v1/models` advertises `context_length`/`max_model_len` at the top level. Claude Code `output_config.effort` / `format: json_schema` on `/v1/messages`. `reasoning_budget_tokens` on chat/Responses (0 off, &gt;0 on) outranks `reasoning_effort`. Legacy OpenAI `functions`/`function_call` map onto tools. Completions `best_of>1` is 400. Think-off closes an open `<think>` (and Muse commits `to=user` when there are no tools). Muse drops prior-turn `to=self` reasoning. `service_tier` flex/scale/priority is 400. `store:true` is 400. `tool_choice` required/any/named appends a last-user must-call-tool hint. Tight `max_tokens`/`num_predict` (&lt;12288) appends a concise-reply hint. `</think>` inside an open tool-call argument is payload. LFM2 truncated pythonic tool calls ship name + `{}`. Missing `</tool_call>` on done still parses the body. `<tool_calls:` (colon) is a Qwen XML wrapper; `<tool_calls>` without colon is content. Laguna bare `{name, arguments}` JSON is only a tool call when the name is declared; tagged `<tool_call>` is not filtered. Gemma tool args that drop `<|"|>` keep commas and braces inside the value. Truncated Gemma `call:name{` ships name + `{}`. Laguna tagged tool JSON that is truncated ships name + `{}`. Glimmer truncated ATEM keeps completed parameters. Harmony truncated tool JSON ships name + `{}`. FunctionGemma missing `<end_function_call>` still parses `call:name{args}` on done. DeepSeek missing `tool▁call▁end` still parses on done (truncated JSON is name + `{}`). Cogito is the same for its fenced `function<sep>name` calls. Ministral truncated `[ARGS]` JSON ships name + `{}`. Olmo3 missing `</function_calls>` still parses on done (truncated `name(args` is name + `{}`). GLM missing `</tool_call>` still parses on done (truncated XML is name + `{}`). Cohere truncated `<|START_ACTION|>` JSON ships name + `{}`. Empty history tool-call ids become stable `call_N`. Mixed thinking+content streams put `logprobs` on the content chunk, not reasoning; reasoning-only drops them. Completions use the four-array OpenAI logprobs shape. Logprob entries include vocab `id` when known. Completions `logprobs:0` still returns chosen tokens (cap 0–5). Unparsed LFM2/Gemma/FunctionGemma/Olmo/`<function_calls>`/ATEM tool wrappers are cut from assistant text. Muse ATEM invoke names keep the trailing identifier. `ZEROLLAMA_MLX_PLD=off` only for benches. See [mlx-serve-borrowings.md](./mlx-serve-borrowings.md).
...
**T=0 greedy trio (mtplx 2.9.2):** draft argmax stays on-device; accept is token equality. Fence 12288 (`ZEROLLAMA_MLX_GREEDY_TRIO_MAX_CONTEXT`; `0` unlimited). Kill: `ZEROLLAMA_MLX_GREEDY_DRAFT_CHAIN=off` / `ZEROLLAMA_MLX_BATCHED_GREEDY_ACCEPT=off`. Sampled requests unchanged. `*.last.json` stamps `greedy_coupled` when both gates fired on a T=0 request. Spec and AR share `prefillBodyLen` (leave one seed token).

**MTP norm convention (mtplx #301):** `+1.0` only when MTP q/k mean max < 1.25 **and** low-set mean min < 0.5. Missing either family or an already-absolute sidecar is left alone. Import no longer `+1`s `mtp.*` just because the tensors exist. Youssofal in-checkpoint packs (`Qwen3.8-27B-MTPLX-*`) can load on lab `:11435` after `zerollama create`; do not bolt their MTP sidecar onto a foreign trunk.
