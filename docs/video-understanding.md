...
Clients may send **`precomputed_embedding`** (post-projector feature rows) with **`padded_input_ids`** instead of PNG **`images[]`**. **Why:** when the client or a GPU worker already ran the vision tower, the server should splice rows at pretokenized vision slots — not decode PNG and re-encode. **Rules:** exactly one precomputed item per message; cannot mix with raw bytes or `processor_output`; requires `padded_input_ids` on the same message. **ollama-engine** implements all native Go VLMs (Qwen/glmocr need `grid_thw` patch grid; Gemma/mllama/deepseekocr rows only; llama4 optional multi-tile grid; lfm2 single-tile). **ggml llamarunner** accepts embed rows on padded inject (Linux auto + Mac vision default). **llama-server** rejects when forced with `ZEROLLAMA_LLAMA_SERVER=1` (400). Grep `precomputed_embedding runner inject`.

### Optional `processor_output` (SGLang HF processor tensors)

Clients may send **`processor_output`** with **`pixel_values`** and **`image_grid_thw`** plus **`padded_input_ids`**. **Why:** SGLang runs the HF processor locally; zerollama should run only vision tower + projector + LLM — skipping PNG decode and processor CPU work. **Rules:** same exclusivity as precomputed; one item per message. **ollama-engine:** Qwen/glmocr patch grids; Gemma3/4/mistral3/llama4/lfm2/deepseekocr pixel canvases (`[1,H,W]` where applicable; **llama4** multi-tile = local + global `image_size²`; **lfm2** tile-grid `[1,rows,cols]` + optional thumbnail; **deepseekocr** tile-grid `[1,rows,cols]` with `640²` locals + `1024²` global, 2–9 tiles). **llamarunner / llama-server** reject. Grep `processor_output runner inject`.
...
**Capacity:** per-session overlay hashes are capped at `OLLAMA_IMAGE_EMBED_CACHE_MAX`. Optional `OLLAMA_IMAGE_EMBED_CACHE_BYTES` (default 0) adds a float32-byte budget on the global ViT LRU (ollama-engine and ggml llamarunner).

**Logs:** `vision embed session cache hit`, `vision embed radix cache hit` / `vision embed global cache hit`, `precomputed_embedding session cache hit` (llamarunner).

**ViT radix (cross-request):** with `OLLAMA_VIT_RADIX` (default on), the same PNG/precomputed hash hits across different `prompt_cache_key`s on one loaded runner — byte budget 100 MiB unless `OLLAMA_IMAGE_EMBED_CACHE_BYTES` is set. Smoke: `VIDEO_AGENT_INFER_VIT_RADIX=1`.

**KV pad_value radix:** `OLLAMA_KV_MM_PAD_RADIX` (default on) rewrites ollama-engine vision pad Tokens to content-hash sentinels so input-cache prefix matching diverges across different images even when vocab `image_pad` ids match. Grep `kv mm pad_value radix applied`.
...
**Runner forward (Jul 2026):** both **client-explicit** and **server ffmpeg** `grid_thw` on `video_spans` forward to mtmd / ollama-engine as per-frame `[1,H,W]` (`GridTHWPerRaster`). On M-RoPE models (Qwen-VL), that triggers `mtmd_bitmap_set_grid_hint` / `EncodeMultimodalWithGrid` resize (`grid_thw hint resize`) so ViT embed count matches preflight. `GridTHWExplicit` remains observability (client vs estimate). Grep `vision grid hint match` when embeds align.
