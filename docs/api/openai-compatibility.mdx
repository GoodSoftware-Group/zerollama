...
Before using a model, pull it locally `zerollama pull`:

```shell
zerollama pull llama3.2
```

### Default model names

For tooling that relies on default OpenAI model names such as `gpt-3.5-turbo`, use `zerollama cp` to copy an existing model name to a temporary name:

```shell
zerollama cp llama3.2 gpt-3.5-turbo
```
...
Use the `zerollama create mymodel` command to create a new model with the updated context size. Call the API with the updated model name:
