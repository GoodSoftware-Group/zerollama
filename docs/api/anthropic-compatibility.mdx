...
```shell
zerollama launch claude --config
```
...
For tooling that relies on default Anthropic model names such as `claude-3-5-sonnet`, use `zerollama cp` to copy an existing model name:

```shell
zerollama cp qwen3-coder claude-3-5-sonnet
```
