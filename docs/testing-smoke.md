...
| `m3_metal_signoff.sh` / `metal_signoff.sh` | Full Mac Metal gate (Phase 13–15). **`RUN_E2E_QWEN35=1`** adds qwen35 **before Phase 15** — **why:** Phase 15 stops the sidecar; qwen35 needs runtime handoff/resume. Canonical tag: **`eliza-1-2b:latest`** (`eliza-1-*` = ship qwen35); `qwen3.6:latest` alternate. **`RUN_E2E_L3=1`** runs prefix-cache smoke — **why:** verify stable `prompt_cache_key` wiring + `/health.llama_cache`. **`smoke_runtime_assert_kv_snapshot`** accepts `bound`+`tensor` when vendor kv-ext linked. M4 Max PASS Jun 2026. |
...
| `l3_radix_prefix_smoke.sh` | Cross-slot Radix prefix share — **why:** same system prompt, different cache keys; offline plan replay + optional `L3_RADIX_LIVE=1` (forces vendor llama-server, probes `/kv/seq-copy`, checks `radix_seed` trace). Mac + 5080 live **PASS** Jun 2026; **4090 `8f114a9b`+0071 live PASS** Jul 2026 (`ZEROLLAMA_RUNTIME_URL=http://127.0.0.1:18081` beside prod). Product gaps: [radix-prefix-share.md](./radix-prefix-share.md#product-gaps). |
...
| `phase15_health_smoke.sh` | Assert `/health` KV keys (`kv_forward_plans`, `kv_page_bind`, `kv_live_physical`, …) via `InferenceEngine` only; when native ext built, `kv_page_bind.status` is `partial` or `not_implemented` (no live decode in this smoke — **why:** GPU smokes assert `bound`+`tensor` after linked kv-ext decode; this CPU-only smoke only checks key presence) |
