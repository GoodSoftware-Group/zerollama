...
| `l3_radix_prefix_smoke.sh` | Cross-slot Radix prefix share — **why:** same system prompt, different cache keys; offline plan replay + optional `L3_RADIX_LIVE=1` (forces vendor llama-server, probes `/kv/seq-copy`, checks `radix_seed` trace). Mac + 5080 live **PASS** Jun 2026. Product gaps: [radix-prefix-share.md](./radix-prefix-share.md#product-gaps). |
