...
<Note>Previously known as Clawdbot. `zerollama launch clawdbot` still works as an alias.</Note>
...
<Note>Web search for local models requires `zerollama signin`.</Note>
...
```bash
zerollama launch openclaw --config
```

To use a specific model directly:

```bash
zerollama launch openclaw --model kimi-k2.5:cloud
```
...
```bash
zerollama launch openclaw --model kimi-k2.5:cloud --yes
```
