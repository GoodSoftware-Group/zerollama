...
| `gpu_found=false` in ~1 ms, `nvidia-smi` OK, binary is `run/zerollama` | Discovery used to glob only `../lib/ollama` and skip `OLLAMA_LIBRARY_PATH` / `/usr/lib/ollama/cuda_v13` | Restart serve on a build with the path.go fix. Confirm `OLLAMA_LIBRARY_PATH` includes `/usr/lib/ollama`. Temporary workaround: `ln -s /usr/lib/ollama ~/zerollama/lib/ollama` |
