...
| **M22** | **UMA HOLD for llama-server Metal** | vendor llama-server + libuma_client | **PoC (Jul 2026)** — patch **0094**; gate `./scripts/phase/m22_llama_server_uma_signoff.sh` (lab `:18082`, incl. **off** escape). Runtime inherits via UMA-built `LLAMA_SERVER_BIN`. Disable: `ZEROLLAMA_UMA_SCHED=off` or `BUILD_UMA=0`. Doc: [llama-server-uma-sched.md](./llama-server-uma-sched.md). |
