...
| **M22** | **UMA HOLD for llama-server Metal** | vendor llama-server + libuma_client | **PoC (Jul 2026)** — patch **0094**: `graph_compute` lease + sync under HOLD; acquire in `llama_backend_init`; `BUILD_UMA=auto` → `libuma_llama.a`. Gate: `./scripts/phase/m22_llama_server_uma_signoff.sh` (lab `:18082`). Runtime inherits when `LLAMA_SERVER_BIN` is this binary. Doc: [llama-server-uma-sched.md](./llama-server-uma-sched.md). |
