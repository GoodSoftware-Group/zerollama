...
| **15** | **Native scheduler + KV** | C/Rust | **Partial (v0–v7 + ops)** — C pool + tick/decode hooks; `kv_bind` + seq-position; `kv_forward_plans` + `/internal/kv-snapshot`; Go `:8080` proxy passthrough for runtime extension fields; scheduler expired/finished deadlock fix. **Next:** tensor KV page bind; batched decode in C. Docs: [phase15-native-kv.md](./phase15-native-kv.md), [handoff-phase15-native-kv.md](./handoff-phase15-native-kv.md). |
