...
| 7 | Coordinate llama.cpp pin with borrowings **L2** (fork KV profiles vs L1 q8_0) | **Partial** — QJL/Polar/TBQ **extracted** (patches 0026–0030 on ggml-org `8f114a9b`); **FAIL default profiles** @ 8k CUDA ship gate (Jun 2026); profiles stay opt-in. `./scripts/phase17_l2_pin_status.sh` |
...
| **L2** | **Fork KV profiles on unified `llama-server`** | Repo + C | **Partial (Jul 2026)** — kernels on ggml-org via patches **0026–0030**; pin `8f114a9b`. **Ship gate FAIL** for default tok/s (5080 eliza-1; 4090 llama3.2). **VRAM opt-in:** `ZEROLLAMA_LLAMA_FORK=1` + `ZEROLLAMA_LLAMA_FORK_PROFILE=vram` (TBQ) — 4090 @ 65k–131k: **−5…−6%** decode for **−27…−35%** VRAM. Doc: [gpu-profiles-l2.md](./gpu-profiles-l2.md). |
