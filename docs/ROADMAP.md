...
| **13** | **Single-GPU + host autoconfig** | Python | **Partial** — estimates, autotune, `suggested_max_num_ctx`, clamp default **off** in YAML; `python -m runtime.gpu_snapshot` after session JSON; `vram:` defaults in `single_gpu.yaml`. **5080 gate:** [gpu-5080-operator-guide.md](./gpu-5080-operator-guide.md). **Next:** per production GGUF autotune on serve. Doc: [phase13-runtime-vram.md](./phase13-runtime-vram.md). |
...
| **15** | **Native scheduler + KV** | C/Rust | **Partial (v0–v7)** — C pool + tick/decode hooks; `kv_bind` + seq-position; `kv_forward_plans` + `/internal/kv-snapshot`. **Next:** tensor KV page bind; batched decode in C. Docs: [phase15-native-kv.md](./phase15-native-kv.md), [handoff-phase15-native-kv.md](./handoff-phase15-native-kv.md). |
