...
| Milestone | Goal | Owner | Exit criteria |
|-----------|------|--------|----------------|
| **L1** | **Per-GPU llama profiles (CUDA + Metal)** | Python | **Partial (Jun 2026)** — `runtime/configs/gpu/*.json` + `runtime/gpu_profiles.py` → `RuntimeConfig.llama_server_args()` (`-b`, `-ub`, `-fa`, `--cache-type-k/v`, `-np`, MTP `draft_*`). **Apple: done** — RAM tiers (`apple_silicon_16g` … `128g`) from `hw.memsize`; M4 Max 128g sign-off (`-np 8`, `-c 131072`). **NVIDIA: partial** — name/VRAM bucket match; **5080 active** (`rtx-5080`, `n_parallel=4`); values eliza-ported until production-GGUF L1 calibration. Fork KV → `q8_0`; fork argv suppressed; `mlock` off by default. Disable: `ZEROLLAMA_GPU_PROFILE=0`. Doc: [gpu-profiles-l1.md](./gpu-profiles-l1.md). |
| **L2** | **`elizaOS/llama.cpp` fork evaluation** | Repo + C | **Partial (Jun 2026)** — sibling build, fork probe, profile unlock, `l2_fork_eval.sh`, `l2_metal_bench.sh`, `l2_cuda_bench.sh`, `l2_full_gate.sh`, `l2_cuda_full_gate.sh`, `linux_runtime_serve_lib.sh`. **Metal:** 8k stock wins decode; 27k/131k warmup fix in. **CUDA 5080 (Jun 2026):** 8k bench run — stock **79.7** vs fork **55.9** tok/s (1B Q8); **FAIL merge**; compat PASS; long-ctx legs pending. **Gate open:** 27k/131k on 5080 + qwen35 ggml smoke. Pin `96dd1a8`. Doc: [gpu-profiles-l2.md](./gpu-profiles-l2.md). |
