...
| Milestone | Goal | Owner | Exit criteria |
|-----------|------|--------|----------------|
| **L1** | **Per-GPU llama profiles (CUDA + Metal)** | Python | **Partial (Jun 2026)** — **Apple: done** (RAM tiers; M4 Max 128g). **NVIDIA 5080: single-stream calibrated** — `rtx-5080.json` tuned on CT 1564 (`n_parallel=2`, `batch_size=1024`, `ubatch_size=256`); 1B Q8 **+0.5%**, eliza-1 9B **+0.7%** vs profile OFF @ 8k (`l1_cuda_calibrate.sh`). **Open:** `l1_cuda_concurrent_bench.sh` — concurrent (N=2) A/B to validate `n_parallel=2` wins under load. Disable: `ZEROLLAMA_GPU_PROFILE=0`. Doc: [gpu-profiles-l1.md](./gpu-profiles-l1.md). |
