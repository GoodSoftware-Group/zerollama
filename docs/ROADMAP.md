...
| **M24** | **GGUF MTP on ggml Metal** | ollamarunner + qwen3next | **Partial (Aug 2026)** — Go engine loads Ollama `mtp.*` and `DraftForward`. **Not done:** speculative accept in ollamarunner; llama-server `draft-mtp` off for `qwen35*` (SWA/GDN). **MLX:** `qwen3.6:27b-mlx-mtp` (`qwen3_5_mtp` DRAFT) speculates on Metal. **Do not** load `qwen3.6-mtp` GGUF on `:11434` (22 GiB). Doc: [apple-silicon-metal.md](./apple-silicon-metal.md), [qwen35-apple-silicon.md](./qwen35-apple-silicon.md). |
