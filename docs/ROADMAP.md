...
| **M20** | **UMA compute broker for MLX** | mlxrunner + bmtl uma_toolkit | **Shipped (Jul 2026)** — **admission only** (not MLX math ops): machine-wide `uma_daemon` via `libuma_client`; `HOLD_GPU`/`RELEASE` while MLX runs Metal. Default **`auto`**; coarse leases; metrics; doctor; **`BUILD_UMA=auto`**. **Gate (Jul 22–23):** golden, agent, HOLD/ATTN/`RUN_HYBRID`/RUN_NOP contention, `cum_*`, half-close reconnect (6b), **broker TERM same-serve recover PASS**. Doc: [mlx-uma-sched.md](./mlx-uma-sched.md). Wishlist: `bmtl/.../uma_toolkit/WISHLIST_MLX.md`. |
| **M21** | **UMA HOLD for GGUF ggml Metal** | ollamarunner + llamarunner + ggml Go | **PoC (Jul 2026)** — ollamarunner `ComputeWithNotify` + llamarunner `Decode`/`Synchronize` under HOLD; coarse **`LeaseBegin(prefill\|decode)`**; projects `ollamarunner-*` / `llamarunner-*`. Gate: `./scripts/phase/m21_ggml_uma_signoff.sh` (lab `:11435`, `m21-ggml` + `llama3.2:3b`; `ZEROLLAMA_LLAMA_SERVER=0`). **Not yet:** runtime Python Metal. Doc: [ggml-uma-sched.md](./ggml-uma-sched.md). |
| **M22** | **UMA HOLD for llama-server Metal** | vendor llama-server + libuma_client | **PoC (Jul 2026)** — patch **0094**: `graph_compute` lease + sync under HOLD; acquire in `llama_backend_init` (one glue copy in `libllama`); `BUILD_UMA=auto` links `libuma_llama.a`. Lab: HOLD wait ≈5s (`wait_ms≈4935`, project `llama-server-*`). Doc: [llama-server-uma-sched.md](./llama-server-uma-sched.md). **Open:** scripted sign-off; runtime sidecar. |
...
| **L2** | **Fork KV profiles on unified `llama-server`** | Repo + C | **Done (Jul 2026)** — kernels on ggml-org via **0026–0030** + **0067–0072**; pin `86d86ed4` (+ follow-ups through **0088**). **Defaults stay L1** (tok/s FAIL merge on Metal/CUDA; Mac eliza-1-2b @ 8k stock **68.8** vs TBQ **48.9**). **VRAM opt-in:** `ZEROLLAMA_LLAMA_FORK=1` (TBQ via `FORK_PROFILE=vram`) or **`ZEROLLAMA_LLAMA_FORK_AUTO_VRAM=1`** when configured ctx ≥ 32768. **Speed** (QJL/Polar) experimental. **Mac done:** Bee B0 (**0087**) smoke PASS; Metal rebuild. **Next (5080):** RotorQuant A/B via `l2_rotorquant_ab.sh` — [5080-runbook.md](./5080-runbook.md#tier-f--rotorquant--post-l2-labs-jul-2026); remap type IDs 44–47 before cherry-pick; B1 adaptive DM deferred — [llama-fork-watchlist.md](./llama-fork-watchlist.md), [gpu-profiles-l2.md](./gpu-profiles-l2.md). Doc: [llama-cpp-unification.md](./llama-cpp-unification.md). |
