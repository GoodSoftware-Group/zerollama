...
| **M21** | **UMA HOLD for GGUF ggml Metal** | ollamarunner + llamarunner + ggml Go | **PoC (Jul 2026)** — ollamarunner `ComputeWithNotify` + llamarunner `Decode`/`Synchronize` under HOLD; coarse leases. Gate: `./scripts/phase/m21_ggml_uma_signoff.sh` (incl. **`ZEROLLAMA_UMA_SCHED=off`** ungated under HOLD). Doc: [ggml-uma-sched.md](./ggml-uma-sched.md). |
| **M22** | **UMA HOLD for llama-server / runtime Metal** | vendor `libllama` + libuma_client | **PoC (Jul 2026)** — patch **0094** on `graph_compute`; gate `m22_llama_server_uma_signoff.sh` (`:18082`, incl. **off**). **Runtime inprocess + subprocess** load the same UMA-linked `libllama.dylib` (ctypes / child). Disable: `ZEROLLAMA_UMA_SCHED=off` or `BUILD_UMA=0`. Doc: [llama-server-uma-sched.md](./llama-server-uma-sched.md), [uma-admission.md](./uma-admission.md). |
