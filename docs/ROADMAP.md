...
| **M21** | **UMA HOLD for GGUF ggml Metal** | ollamarunner + ggml Go | **PoC (Jul 2026)** — reuse M20 `libuma_client` glue: `ComputeWithNotify` runs `graph_compute_async` + **eager** `sched_synchronize` under `HOLD_GPU` (no RELEASE race). Project `ollamarunner`. Same `ZEROLLAMA_UMA_SCHED` / `BUILD_UMA`. **Lab:** `eliza-1-2b` generate PASS on `:11435` with `require`. **Not yet:** llama-server / runtime Metal; C++ metal wrap; coarse phase leases. Doc: [ggml-uma-sched.md](./ggml-uma-sched.md). Wishlist: `WISHLIST_GGML.md`. |
