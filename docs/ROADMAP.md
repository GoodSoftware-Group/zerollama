...
| Milestone | Goal | Owner | Exit criteria |
|-----------|------|--------|----------------|
| **L1** | **Per-GPU llama profiles (CUDA + Metal)** | Python | **Partial (Jun 2026)** — **Apple: done** (RAM tiers; M4 Max 128g). **NVIDIA 5080: calibrated + concurrent PASS** — `rtx-5080.json` (`n_parallel=2`, `batch_size=1024`, `ubatch_size=256`); single-stream 1B **+0.5%**, 9B **+0.7%** @ 8k (`l1_cuda_calibrate.sh`); **concurrent N=2** ON **102.7** vs OFF **92.9** agg tok/s (**+10.5%**, `l1_cuda_concurrent_bench.sh`; OFF leg 1×502 expected at `n_parallel=1`). **Open:** supernova-class GGUF re-calibrate. Disable: `ZEROLLAMA_GPU_PROFILE=0`. Doc: [gpu-profiles-l1.md](./gpu-profiles-l1.md). |
...
| **L3** | **Prompt cache key → slot bridge** | Go + Python | **Partial (Jun 2026)** — pinned slots, subprocess + in-process RAM resume, **in-process disk parity** (`llama_state_seq_*`), batch keys, `/health.llama_cache`, smokes. **5080 (Jun 2026):** **STRICT PASS** @ 8k on eliza-1 9B (`l3_cache_smoke.sh`, cached **−42%** vs no-cache); **`l3_production_gate.sh` PASS** @ 27k (cached **0.72s** vs no-cache **1.48s**; strict ratio **1.02** — open on supernova). 1B Q8 SOFT PASS. Doc: [gpu-profiles-l3.md](./gpu-profiles-l3.md). |
