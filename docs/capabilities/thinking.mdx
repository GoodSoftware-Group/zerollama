...
- Enable thinking for a single run: `zerollama run deepseek-r1 --think "Where should I visit in Lisbon?"`
- Disable thinking: `zerollama run deepseek-r1 --think=false "Summarize this article"`
- Hide the trace while still using a thinking model: `zerollama run deepseek-r1 --hidethinking "Is 9.9 bigger or 9.11?"`
- Inside interactive sessions, toggle with `/set think` or `/set nothink`.
- GPT-OSS only accepts levels: `zerollama run gpt-oss --think=low "Draft a headline"` (replace `low` with `medium` or `high` as needed).
