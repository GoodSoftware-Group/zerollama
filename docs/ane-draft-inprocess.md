...
**Open (B7):** `ZEROLLAMA_ANE_DRAFT_DRIVE=shadow|force` — ANE matmul output → host tied-embed argmax → draft token (`shadow` logs parity, `force` replaces Metal sampler). **P6 token shadow (M4 Max, Jun 2026):** `shadow_steps=14`, `shadow_match_pct=0%` (expected — blk.0 proxy ≠ full dflash), `shadow_hidden_cos≈1.0` on ffn_down stash vs CPU golden. **Force baseline (P6):** ~27% draft acceptance, large TPS hit from sync eval per handoff. **Open (B8):** target-arch graph concat for `t_dflash_target_features` (export API wired; qwen35 still logs `no dflash target features`); Metal acceptance still ~0% on short lab smokes — diagnose verify path next. **Shipped (Jul 2026):** `common_ane_draft_sync_target_cross` runs after each target decode in speculative `process()` so native Metal dflash reads `cross.v_embd` before draft sync-decode. **Shipped (Jul 12 2026):** sync fills `cross.v_embd` from real `llama_get_embeddings_layer_inp` (same gather as `process()` KV inject) instead of tiling pre-norm across layer slots; `ZEROLLAMA_ANE_DRAFT_TARGET_FEAT_TILE>0` forces the old stub for A/B. **Shipped (Jul 12 2026):** `--spec-type dflash` alias → `draft-dflash` (zerollama serve was passing `dflash` while the pin only accepted `draft-dflash`). **Partial (Jul 12 2026):** `llama_set_dflash_target_export(ctx_tgt, model_dft)` called from dflash ctor; graph still must assign `t_dflash_target_features` for export rows to appear.
