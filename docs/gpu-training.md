...
```bash
./scripts/training/training_uv_venv.sh --verify
# several uv binaries on PATH:
UV_BIN="$HOME/.local/bin/uv" ./scripts/training/training_uv_venv.sh --verify
eval "$(./scripts/training/training_uv_venv.sh --export)"
zerollama serve
```

Or use `./scripts/serve/serve_mac_runtime.sh` — it picks up `.venv-training` automatically when present.
...
TRAINING_UV_PYTHON_VER="$(./scripts/training/training_uv_venv.sh --embed-py)"
TRAINING_UV_VENV="$PWD/.venv-training" TRAINING_UV_PYTHON_VER="${TRAINING_UV_PYTHON_VER}" \
  ./scripts/training/training_uv_venv.sh --verify
eval "$(TRAINING_UV_VENV=$PWD/.venv-training ./scripts/training/training_uv_venv.sh --export)"
```
...
**Production serve:** install [`scripts/serve/serve_production_wrapper.sh`](../scripts/serve/serve_production_wrapper.sh) as `~/bin/serve.sh` — **WHY not copy `serve_gpu_example.sh` verbatim:** `~/bin` breaks repo-root detection (`_ROOT=$HOME`); embed/training env never loads. The wrapper `exec`s the in-repo example, which sets `TRAINING_UV_SITE_PACKAGES` from `ldd zerollama` before `zerollama serve`. CT 1564: `cp scripts/serve/serve_production_wrapper.sh ~/bin/serve.sh && chmod +x ~/bin/serve.sh`.

**After migrating to 3.11 (disk cleanup):** once `ldd zerollama` shows `libpython3.11` and `./scripts/training/training_uv_venv.sh --verify` passes, remove duplicate 3.10 trees — each full torch+CUDA venv is ~7 GiB.
...
| `TRAINING_UV_PYTHON_VER` | uv `--python` spec (default: `ldd zerollama` libpython, else `pkg-config python3-embed`, else `3.11`). **Why not pkg-config alone:** may disagree with the linked binary on partial upgrades. Print only: `./scripts/training/training_uv_venv.sh --embed-py`. |
| `TRAINING_EMBED_PY` | Set by [`training_embed_build_env.sh`](../scripts/training/training_embed_build_env.sh) when sourcing before `go build` |
...
```bash
UV_BIN=uv ./scripts/training/training_uv_venv.sh --verify
eval "$(./scripts/training/training_uv_venv.sh --export)"
```
...
**Smoke:** `OLLAMA_HOST=http://127.0.0.1:8080 ./scripts/e2e/e2e_training_ops_smoke.sh` and  
...
| `set OLLAMA_TRAINING_PYTHONPATH` | No `training.py` found via discovery (binary walk, cwd walk, `$HOME/zerollama`). Example: [`scripts/serve/serve_gpu_example.sh`](../scripts/serve/serve_gpu_example.sh) via [`serve_production_wrapper.sh`](../scripts/serve/serve_production_wrapper.sh). |
| `training_init failed` / import errors | Missing Python deps (`torch`, …). Run [`./scripts/training/training_uv_venv.sh --verify`](../scripts/training/training_uv_venv.sh). |
| `training worker not started` / `embedded Python X.Y requires .venv-training/lib/pythonX.Y/site-packages` | **ABI mismatch or missing venv:** binary embeds one libpython, `.venv-training` has another (e.g. 3.11 venv, 3.10 binary). **Fix:** `TRAINING_UV_PYTHON_VER="$(./scripts/training/training_uv_venv.sh --embed-py)" TRAINING_UV_VENV=$REPO/.venv-training ./scripts/training/training_uv_venv.sh --verify`, restart serve. **Why not `venv-training/`:** legacy path; shim and serve scripts expect `.venv-training`. |
| `No module named 'torch'` with `(.venv)` active | Training uses `PYTHONPATH` from `.venv-training`, not an activated shell venv. Run `./scripts/training/training_uv_venv.sh --export` before `serve`. |
| `Failed to load PyTorch C extensions` / `torch/_C` folder | **Python ABI mismatch:** Go embeds `libpython3.X` but `.venv-training` was built for `3.Y`. Recreate: `TRAINING_UV_PYTHON_VER="$(./scripts/training/training_uv_venv.sh --embed-py)" ./scripts/training/training_uv_venv.sh --verify` then restart (rebuild zerollama only if you changed which libpython you link against). |
...
| Embedded runtime `GET /health` hangs (training on) | Known shared-interpreter issue: [`docs/bugs/shared-interpreter-health-hang.md`](bugs/shared-interpreter-health-hang.md), repro [`scripts/runtime/repro_shared_interpreter_health_hang.sh`](../scripts/runtime/repro_shared_interpreter_health_hang.sh). Workaround: `OLLAMA_TRAINING=false` or external runtime (`ZEROLLAMA_RUNTIME_URL` + `ZEROLLAMA_RUNTIME_EMBED=0`). |
| `free(): invalid pointer` at training step 0 (5080) | Mismatched libcudnn: serve sets `/usr/hostlibs` for ggml while embedded torch needs bundled cuDNN in `.venv-training/.../torch/lib`. **Fix:** rebuild `zerollama` so `training_shim.c` runs `embedded_prepare_pytorch_ld_path()` before `Py_Initialize` ([`x/pyembed_common/embedded_pytorch_env.c`](../x/pyembed_common/embedded_pytorch_env.c)). Confirm log line `prepended .../torch/lib to LD_LIBRARY_PATH`. Ensure `eval "$(./scripts/training/training_uv_venv.sh --export)"` before serve. |
...
| Area | Location |
|------|----------|
| Training venv (uv) | [`scripts/training/training_uv_venv.sh`](../scripts/training/training_uv_venv.sh) — `--embed-py`, `--verify`, `--export` |
| CGO embed version (Linux) | [`scripts/training/training_embed_build_env.sh`](../scripts/training/training_embed_build_env.sh) — PKG_CONFIG overlay for `libpython3.11` |
