...
```shell
OLLAMA_CONTEXT_LENGTH=8192 zerollama serve
```

To change this when using `zerollama run`, use `/set parameter`:
...
Use the `zerollama ps` command to see what models are currently loaded into memory.
...
OLLAMA_ORIGINS=chrome-extension://*,moz-extension://*,safari-web-extension://* zerollama serve
```
...
```shell
zerollama run llama3.2 ""
```

## How do I keep a model loaded in memory or make it unload immediately?

By default models are kept in memory for 5 minutes before being unloaded. This allows for quicker response times if you're making numerous requests to the LLM. If you want to immediately unload a model from memory, use the `zerollama stop` command:

```shell
zerollama stop llama3.2
```
