...
- [Inference wishlist (host Phase A)](docs/inference-wishlist-host.md) — capacity APIs, admission Retry-After, empty-gen vs host_unstable
...
**Capacity & admission (wishlist Phase A, Jul 2026):** agents probe **`GET /api/status`** → `inference.config` (parallel slots, max loaded, queues, keep-alive), dry-run **`POST /api/can-load`** (never starts a load; runtime VRAM `exact` vs ggml `heuristic`; fail closed if estimate missing), scrape **`GET /api/metrics`**, and honor **`Retry-After`** on busy 503s. Empty done chunks use `done_reason=empty_generation`; runner deaths use `cause=host_unstable`. **Why:** Orient Inventory / Decide need capacity without guessing env or thrashing swaps. Stable multi-model pin/swap stays Phase B. Guide: [inference-wishlist-host.md](docs/inference-wishlist-host.md).
