...
- [L3 prompt cache → slot bridge](docs/gpu-profiles-l3.md) — **why** stable session keys skip repeat prefill; pinned `id_slot` + disk TTL; batch `prompt_cache_keys`; **5080 Jun 2026:** STRICT PASS on eliza-1 9B (long prefix + no-cache compare).
...
- [L1 GPU profiles (autotune)](docs/gpu-profiles-l1.md) — **why** Phase 13 ≠ throughput tuning; NVIDIA name/VRAM buckets; Apple RAM tiers; **5080 `l1_cuda_calibrate.sh`**; stock llama.cpp safety; operator env.
