...
**MLX safetensors (`gemma4`, Hermes agents):** long agent prompts are capped and tail-truncated server-side; pass `options.num_ctx` ≤ model max. Pass a stable **`prompt_cache_key`** per thread so turn 2+ can hit **live-session** (`fast_path=true` in `gemma-agent.jsonl`) or trie restore — without the key, 99% `cached_tokens` can still mean ~60–90s TTFT on OptiQ. If `messages_dropped` rises between turns, expect partial cache (~16k), not full reuse. Check logs for `num_ctx capped`, `prompt tail-truncated`, `prefill complete`, `prompt_chain_miss` `messages_truncated`. Guide: [mlx-agent-prompts.md](docs/mlx-agent-prompts.md).
