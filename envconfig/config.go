...
func AsMap() map[string]EnvVar {
	ret := map[string]EnvVar{
		...
		"ZEROLLAMA_GGML_CLAMP_NUM_CTX":        {"ZEROLLAMA_GGML_CLAMP_NUM_CTX", ggmlClampNumCtxDisplay(), "Lower ggml merged num_ctx to suggested_max before load (default off; parity with runtime VRAM clamp)"},
		...
		"ZEROLLAMA_GGML_VRAM_MARGIN":          {"ZEROLLAMA_GGML_VRAM_MARGIN", Var("ZEROLLAMA_GGML_VRAM_MARGIN"), "Multiply ggml load VRAM estimate before compare to free memory (default 1.05)"},
