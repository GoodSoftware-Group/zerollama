...
// OVERLAP helpers (dylib pack_eval_async + Metal unpack). Lab only.
...
// Holey OVERLAP (ZEROLLAMA_ANE_FFN_OVERLAP=1): after Metal sync, kick ANE pack+eval
// async so MoE can encode in parallel. Pair with _finish at shexp down.
