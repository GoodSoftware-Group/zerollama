...
static bool fuse_shared_ok(const ane_ffn_swiglu_fuse_t * fuse, const char *wname) {
    if (!fuse || !fuse->up || !fuse->gate || !fuse->down || !fuse->dst || fuse->n_fuse < 4) {
        return false;
    }
    if (!tensor_host_shared(fuse->up->src[0]) ||
        !tensor_host_shared(fuse->gate->src[0]) ||
        !tensor_host_shared(fuse->down->src[0]) ||
        !tensor_host_shared(fuse->up->src[1]) ||
        !tensor_host_shared(fuse->dst)) {
        ane_ffn_force_note_bail("shared", fuse->ic, fuse->hidden, fuse->seq, wname);
        return false;
    }
    return true;
}

static void widen_f16_to_f32_ggml(const uint16_t *s, float *d, size_t n) {
    for (size_t i = 0; i < n; i++) {
        uint16_t h = s[i];
        const uint32_t sign = (uint32_t)(h >> 15) << 31;
        const uint32_t exp  = (h >> 10) & 0x1f;
...
// Kick ANE pack+eval after Metal sync (ANE∥MoE). Warm INT8_IN sessions only;
// cold → false so deferred down does sync try_swiglu_tensors (quality-safe).
bool ane_ffn_force_try_swiglu_tensors_kick(const ane_ffn_swiglu_fuse_t * fuse) {
    if (!env_on("ZEROLLAMA_ANE_FFN_INT8_IN")) {
        return false;
    }
    if (!fuse || !fuse->up) {
        return false;
    ...
    const char *wname = fuse->up->src[0] ? fuse->up->src[0]->name : NULL;
    if (!fuse_shared_ok(fuse, wname)) {
        return false;
    }
    const int ic = fuse->ic;
    const int hidden = fuse->hidden;
    const int seq = fuse->seq;
    const int a_f16 = type_is_f16(fuse->up->src[1]->type);

    float *Wg = NULL;
    float *Wu = NULL;
    float *Wd = NULL;
    if (!wcache_ensure(fuse, &Wg, &Wu, &Wd)) {
        ane_ffn_force_note_bail("wcache_kick", ic, hidden, seq, wname);
        return false;
    }
    ane_ffn_force_swiglu_bind_weight_ids(
        fuse->gate->src[0]->data, fuse->up->src[0]->data, fuse->down->src[0]->data);
    if (!ane_ffn_force_swiglu_activate_session(ic, hidden, seq, Wg, Wu, Wd)) {
        return false;
    }
    if (!ane_ffn_force_swiglu_pack_eval_async_host(
            fuse->up->src[1]->data, ic, seq, a_f16)) {
        ane_ffn_force_note_bail("kick_async", ic, hidden, seq, wname);
        return false;
    }
    return true;
}

bool ane_ffn_force_try_swiglu_tensors_finish(const ane_ffn_swiglu_fuse_t * fuse) {
    if (!fuse || !fuse->up || !fuse->dst) {
        return false;
    }
    const char *wname = fuse->up->src[0] ? fuse->up->src[0]->name : NULL;
    const int ic = fuse->ic;
    ...
    if (!ane_ffn_force_swiglu_pack_eval_async_wait_host()) {
        ane_ffn_force_note_bail("overlap_wait", ic, hidden, seq, wname);
        return false;
    }
    if (!acache_ensure(ic, seq)) {
        ane_ffn_force_note_bail("acache_finish", ic, hidden, seq, wname);
        return false;
    }
    struct ggml_tensor * dst = (struct ggml_tensor *)fuse->dst;
    if (a_f16) {
        if (!ane_ffn_force_swiglu_unpack_metal_to_ggml_f16(ic, seq, dst->data)) {
            ane_ffn_force_note_bail("overlap_unpack", ic, hidden, seq, wname);
            return false;
        }
        ane_ffn_profile_tick_replace();
        return true;
    }
    if (!ane_ffn_force_swiglu_unpack_metal_to_ggml_f16(ic, seq, g_acache.Y16)) {
        ane_ffn_force_note_bail("overlap_unpack", ic, hidden, seq, wname);
        return false;
    }
    widen_f16_to_f32_ggml(g_acache.Y16, (float *)dst->data, (size_t)ic * (size_t)seq);
    ane_ffn_profile_tick_replace();
    return true;
}
