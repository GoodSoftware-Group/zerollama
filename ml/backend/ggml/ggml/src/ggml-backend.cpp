...
ggml_backend_buffer_t ggml_backend_buft_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    if (buft->no_alloc) {
        // Ollama fit path: size tensors/graph without backing device memory.
        // Go LoadOperationFit passes alloc_buffers=false via sched_new_ext.
...
ggml_backend_sched_t ggml_backend_sched_new_ext(
        ...
        bool op_offload,
        bool alloc_buffers) {
    // alloc_buffers=false: set buft->no_alloc so reserve/alloc_graph report sizes only.
    // Ollama removed upstream sched_set_batch_size; this replaces the old fit-only path.
...
bool ggml_backend_sched_reserve(ggml_backend_sched_t sched, struct ggml_cgraph * measure_graph) {
    ...
    for (int i = 0; i < sched->n_splits; i++) {
        ...
        if (split_backend->iface.graph_reserve != NULL) {
            // Backend-specific reserve (e.g. CUDA graph capture sizing) when not no_alloc.
