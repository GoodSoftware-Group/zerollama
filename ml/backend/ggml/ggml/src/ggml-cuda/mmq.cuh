...
static constexpr __host__ __device__ tile_x_sizes mmq_get_dp4a_tile_x_sizes(ggml_type type, int mmq_y) {
    switch (type) {
        ...
        case GGML_TYPE_FP8_E4M3: return MMQ_DP4A_TXS_Q8_0;
        case GGML_TYPE_FP8_E5M2: return MMQ_DP4A_TXS_Q8_0;
...
static constexpr __host__ __device__ int mmq_get_mma_tile_x_k(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_FP8_E4M3: return MMQ_MMA_TILE_X_K_Q8_0;
        case GGML_TYPE_FP8_E5M2: return MMQ_MMA_TILE_X_K_Q8_0;
...
// WHY amax→int8: sm_89 has no native FP8 tensor-core MMA in this stack; reuse Q8_0 DP4A/MMA
// tiles after a cheap per-block requant. Scale absorbs F16 block d * (amax/127).
static __device__ __forceinline__ int pack_fp8_e4m3_qs_as_q8(const block_fp8_e4m3 * bxi, const int kqs) {
    float amax = 0.0f;
#pragma unroll
    for (int j = 0; j < QK_FP8_E4M3; ++j) {
        amax = fmaxf(amax, fabsf(ggml_cuda_fp8_e4m3_to_fp32(bxi->qs[j])));
    }
    const float mul = amax > 0.0f ? 127.0f / amax : 0.0f;
    int packed = 0;
#pragma unroll
    for (int j = 0; j < 4; ++j) {
        const float f = ggml_cuda_fp8_e4m3_to_fp32(bxi->qs[4*kqs + j]) * mul;
...
static __device__ __forceinline__ float fp8_e4m3_block_d_as_q8(const block_fp8_e4m3 * bxi) {
    float amax = 0.0f;
#pragma unroll
    for (int j = 0; j < QK_FP8_E4M3; ++j) {
        amax = fmaxf(amax, fabsf(ggml_cuda_fp8_e4m3_to_fp32(bxi->qs[j])));
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_fp8_e4m3(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    float * x_df = (float *) (x_tile + 2*MMQ_TILE_NE_K);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_FP8_E4M3, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    ...
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
    const int kbx  = txi / QI_FP8_E4M3;
    const int kqsx = txi % QI_FP8_E4M3;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const block_fp8_e4m3 * bxi0 = (const block_fp8_e4m3 *) x + kbx0 + i*stride + kbx;
        const block_fp8_e4m3 * bxi1 = bxi0 + MMQ_TILE_NE_K/QI_FP8_E4M3;

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + 0             + txi] = pack_fp8_e4m3_qs_as_q8(bxi0, kqsx);
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + MMQ_TILE_NE_K + txi] = pack_fp8_e4m3_qs_as_q8(bxi1, kqsx);
#else
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + 0             + txi] = pack_fp8_e4m3_qs_as_q8(bxi0, kqsx);
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + MMQ_TILE_NE_K + txi] = pack_fp8_e4m3_qs_as_q8(bxi1, kqsx);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    }

    constexpr int blocks_per_tile_x_row = 2*MMQ_TILE_NE_K / QI_FP8_E4M3;
    constexpr int rows_per_warp = warp_size / blocks_per_tile_x_row;
    const int kbxd = threadIdx.x % blocks_per_tile_x_row;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * rows_per_warp) {
        ...
        const block_fp8_e4m3 * bxi = (const block_fp8_e4m3 *) x + kbx0 + i*stride + kbxd;
        const float d = fp8_e4m3_block_d_as_q8(bxi);

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        x_df[i*MMQ_MMA_TILE_X_K_Q8_0                 + kbxd] = d;
#else
        x_df[i*(2*MMQ_TILE_NE_K/QI_FP8_E4M3) + i/(QI_FP8_E4M3/2) + kbxd] = d;
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    }
}
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_fp8_e5m2(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    float * x_df = (float *) (x_tile + 2*MMQ_TILE_NE_K);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_FP8_E5M2, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
...
#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + 0             + txi] = pack_fp8_e5m2_qs_as_q8(bxi0, kqsx);
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + MMQ_TILE_NE_K + txi] = pack_fp8_e5m2_qs_as_q8(bxi1, kqsx);
#else
...
template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_FP8_E4M3> {
    static constexpr int              vdr          = VDR_FP8_E4M3_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_fp8_e4m3<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, MMQ_Q8_1_DS_LAYOUT_D4>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_q8_1_dp4a<mmq_x, mmq_y>;
};

template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_FP8_E5M2> {
    static constexpr int              vdr          = VDR_FP8_E5M2_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_fp8_e5m2<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, MMQ_Q8_1_DS_LAYOUT_D4>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_q8_1_dp4a<mmq_x, mmq_y>;
};
...
extern DECL_MMQ_CASE(GGML_TYPE_FP8_E4M3);
extern DECL_MMQ_CASE(GGML_TYPE_FP8_E5M2);
