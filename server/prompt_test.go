...
{{- if .Response }}{{ .Response }} {{ end }}`)
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			think := false
			prompt, images, _, _, _, err := chatPrompt(t.Context(), &model, mockRunner{}.Tokenize, &opts, tt.msgs, nil, &api.ThinkValue{Value: think}, tt.truncate, 0, nil)
...
{{- if .Response }}{{ .Response }} {{ end }}`)
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			opts := api.Options{Runner: api.Runner{NumCtx: tt.limit}}
			think := false
			_, _, _, _, _, err := chatPrompt(t.Context(), &model, countingTokenize, &opts, tt.msgs, nil, &api.ThinkValue{Value: think}, true, 0, nil)
			if err != nil {
				t.Fatal(err)
			}
...
func TestChatPromptRendererDoesNotRewriteMessageContent(t *testing.T) {
	...
	prompt, images, _, _, _, err := chatPrompt(t.Context(), &m, mockRunner{}.Tokenize, &opts, msgs, nil, &api.ThinkValue{Value: think}, true, 0, nil)
	if err != nil {
		t.Fatal(err)
	}
